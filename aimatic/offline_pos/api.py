...
def _require_open_entry_for_cashier(opening_entry, cashier_user, pos):
	...
	if entry.user != cashier_user:
		frappe.throw(
			_("POS Opening Entry {0} does not belong to cashier {1}").format(opening_entry, cashier_user),
			frappe.PermissionError,
		)
...
def provision_terminal_credentials(username, password):
	...
	if not frappe.db.exists("User", username):
		_audit_pos_admin_action(
			None, "provision_terminal_credentials", "electron-provisioning", "invalid_user"
		)
		frappe.throw(_("Invalid credentials"), frappe.AuthenticationError)

	if not cint(frappe.db.get_value("User", username, "enabled")):
		_audit_pos_admin_action(
			username, "provision_terminal_credentials", "electron-provisioning", "disabled_user"
		)
		frappe.throw(_("Invalid credentials"), frappe.AuthenticationError)
	...
	if not roles.intersection(_ALLOWED_CASHIER_ROLES):
		_audit_pos_admin_action(
			username, "provision_terminal_credentials", "electron-provisioning", "missing_role"
		)
...
def _get_pos_android_oauth_client_id():
	...
	"""
	client_id = frappe.db.get_value("OAuth Client", {"app_name": _POS_ANDROID_OAUTH_APP_NAME}, "name")
	if not client_id:
...
def require_active_device(hardware_id):
	...
	device = frappe.db.get_value("POS Device", hardware_id, ["enabled", "pos_profile"], as_dict=True)
	if not device:
...
def generate_device_enrollment_code(pos_profile):
	...
	enrollment_value = json.dumps({"url": site_url, "token": token}, separators=(",", ":"))
	return {
		"url": site_url,
		"token": token,
		"pos_profile": pos.name,
...
def redeem_device_enrollment(token, hardware_id):
	...
	token_hash = _hash_device_token(token)
	enrollment_name = frappe.db.get_value("POS Device Enrollment", {"token_hash": token_hash}, "name")
...
def _validate_nonzero_rates(doc):
	...
	for row in doc.items:
		if flt(row.net_rate) <= 0:
			frappe.throw(
				_(
					"Rate must be greater than zero for item {0}. Check that it has an active price for this branch."
...
def _build_pos_invoice_doc(
	...
	for it in items:
		...
		if not item_code:
			frappe.throw(_("Item code not provided and barcode not found: {0}").format(barcode))

		item_flags = frappe.db.get_value("Item", item_code, ["disabled", "is_sales_item"], as_dict=True)
		if not item_flags:
...
def preview_cart(
	...
	doc = _build_pos_invoice_doc(pos, cust, items, coupon_code, redeem_loyalty_points, loyalty_points)
	...
	value_excluding_tax = flt(sum(flt(r.get("custom_fbr_value_excluding_tax") or 0) for r in rows), 2)
	total_sales_tax = flt(sum(flt(r.get("custom_fbr_sales_tax") or 0) for r in rows), 2)
	...
	if gift_voucher_code:
		...
		try:
			...
			if payable_before_voucher < flt(criteria.minimum_redemption_value, 2):
				gift_voucher_error = _("This sale must be at least {0} to redeem this gift voucher").format(
					criteria.minimum_redemption_value
				)
			else:
				gift_voucher_amount = min(flt(gv.amount, 2), max(0.0, payable_before_voucher))
...
def submit_online_sale(
	...
	if cint(offline_authenticated) and offline_auth_method != _OFFLINE_AUTH_METHOD_PIN:
		frappe.throw(
			_("Offline-authenticated sales must use offline_auth_method '{0}'; got '{1}'").format(
				_OFFLINE_AUTH_METHOD_PIN, offline_auth_method
			),
			frappe.PermissionError,
		)
...
def _validate_and_set_payments(doc, pos, payments_data, gift_voucher_amount=0):
	...
	credit_rows = [
		p for p in payments_data if (p.get("mode_of_payment") or "").strip() == _FOOD_PANDA_CREDIT_MODE
	]
	if credit_rows:
	...
	def _is_cash(mode):
		if mode not in _mop_type_cache:
			_mop_type_cache[mode] = frappe.db.get_value("Mode of Payment", mode, "type") or "General"
	...
	for p in payments_data:
		...
		if mode not in allowed_modes:
			frappe.throw(_("Payment mode '{0}' is not allowed in POS Profile {1}").format(mode, pos.name))

		if amount <= 0:
			frappe.throw(_("Payment amount must be greater than zero (mode: {0})").format(mode))

		if not _is_cash(mode):
			remaining = flt(payable - total_paid, 2)
			if amount > flt(remaining + 0.005, 2):
				frappe.throw(
					_("Non-cash payment '{0}' of {1} exceeds remaining payable {2}").format(
						mode, amount, remaining
					)
				)
	...
	if total_paid < flt(payable - 0.005, 2):
		frappe.throw(_("Total payments {0} are less than the payable amount {1}").format(total_paid, payable))
	...
	doc.base_change_amount = flt(doc.change_amount * flt(doc.get("conversion_rate") or 1), 2)
...
def _validate_cashier_for_sale(cashier_user, pos):
	...
	"""
	if not frappe.db.exists("User", cashier_user):
		frappe.throw(_("Cashier {0} is not a valid user").format(cashier_user), frappe.PermissionError)
...
def _build_submission_response(doc):
	...
	return {
		...
		"total_taxes_and_charges": flt(getattr(doc, "total_taxes_and_charges", 0) or 0, 2),
...
def get_customer_benefits(pos_profile, customer):
	...
	details = get_loyalty_program_details_with_points(cust.name, loyalty_program, company=company)
...
def close_pos_session(opening_entry, cashier_user, closing_balances, notes=None, supervisor_token=None):
	...
	cashier_user = _resolve_cashier_user(cashier_user, opening.pos_profile, None, is_bearer)

	if opening.user != cashier_user:
		frappe.throw(
			_("POS Opening Entry {0} does not belong to cashier {1}").format(opening.name, cashier_user),
			frappe.PermissionError,
		)
	...
	if not cashier_roles.intersection(_ALLOWED_CLOSE_SHIFT_ROLES):
		...
		_consume_pos_admin_authorization_token(
			supervisor_token, action="close_shift", terminal_id=terminal_id
		)

	if opening.company != pos.company:
		frappe.throw(_("POS Opening Entry {0} does not match its POS Profile company.").format(opening.name))
	...
	if opening.docstatus != 1:
		frappe.throw(
			_("POS Opening Entry {0} must be submitted before it can be closed.").format(opening.name)
		)
	...
	if existing:
		...
		if existing.docstatus == 1 and existing.status == "Failed":
			...
			if failed.status == "Failed":
				frappe.throw(
					failed.error_message or _("POS Closing Entry {0} failed again.").format(failed.name)
				)
			return _build_closing_session_response(failed, opening.name)
		frappe.throw(
			_(
				"A draft POS Closing Entry {0} already exists for this shift. Please resolve it before closing again."
	...
	for row in closing_balances:
		...
		try:
			...
			frappe.throw(_("closing_amount must be a number for payment mode '{0}'").format(mode_of_payment))
	...
	reconciliation_by_mode = {row.mode_of_payment: row for row in closing.payment_reconciliation}
	for balance in opening.get("balance_details") or []:
		...
		if reconciliation:
			...
			reconciliation.expected_amount = flt(reconciliation.expected_amount) + flt(balance.opening_amount)
		else:
	...
	if unknown_modes:
		frappe.throw(
			_("Payment mode(s) are not part of this POS shift: {0}").format(", ".join(sorted(unknown_modes)))
		)
...
def get_pos_closing_summary(opening_entry, cashier_user):
	...
	if not opening_profile:
		frappe.throw(_("Invalid POS Opening Entry: {0}").format(opening_entry))
	cashier_user = _resolve_cashier_user(cashier_user, opening_profile, None, is_bearer)
	...
	return {
		...
		"cashierFullName": frappe.db.get_value("User", closing_entry.user, "full_name") or closing_entry.user,
		...
		"totalOpening": flt(
			sum(flt(p.opening_amount) for p in closing_entry.get("payment_reconciliation") or []),
			2,
		),
		"totalExpected": flt(
			sum(flt(p.expected_amount) for p in closing_entry.get("payment_reconciliation") or []),
			2,
		),
		"isEstimate": False,
	}
...
def start_pos_session(pos_profile, cashier_user, opening_balances=None):
	...
	if not allowed_modes:
		frappe.throw(_("No payment methods are configured in POS Profile: {0}").format(pos.name))

	if not opening_balances:
		# Default: one zero-amount row for every payment mode in the profile
		balance_rows = [
			{"mode_of_payment": p.mode_of_payment, "opening_amount": 0.0} for p in profile_payments
		]
	else:
		balance_rows = []
		for row in opening_balances:
			...
			if mode not in allowed_modes:
				frappe.throw(
					_("Payment mode '{0}' is not configured in POS Profile {1}").format(mode, pos.name)
				)
...
def _get_fbr_service_fee():
	...
	if not settings_name:
		return 0.0
	return flt(
		frappe.db.get_value("FBR Integration Settings", settings_name, "fbr_pos_fee_amount") or 0,
		2,
	)
...
def _pos_profile_user_allowed(pos, fieldname, user):
	...
	allowed_users = {
		getattr(row, "user", None) for row in (pos.get(fieldname) or []) if getattr(row, "user", None)
	}
...
def _require_refund_permission(pos, user=None, supervisor_token=None):
	...
	if allowed is False:
		frappe.throw(
			_("Refund is not allowed for user {0} on POS Profile {1}").format(user, pos.name),
			frappe.PermissionError,
		)
	...
	_require_https_for_pos_admin_authorization()
	_consume_pos_admin_authorization_token(supervisor_token, action="refund", terminal_id=terminal_id)
...
def _validate_refund_quantities(original, requested):
	...
	for row_name, qty in requested.items():
		...
		if not row:
			frappe.throw(_("Row {0} does not belong to invoice {1}").format(row_name, original.name))
		if qty <= 0:
		...
		if qty > flt(remaining + 0.001, 3):
			frappe.throw(
				_("Return quantity {0} exceeds remaining {1} for {2}").format(qty, remaining, row.item_code)
			)
...
def _validate_return_doc_against_original(return_doc, original):
	...
	for row in return_doc.items:
		...
		if not original_row:
			frappe.throw(_("Return row {0} is not linked to the original invoice").format(row.item_code))
...
def _normalize_refund_fbr_status(status, invoice_number):
	...
	has_number = bool(invoice_number) and str(invoice_number).strip().lower() not in _FBR_INVALID_NUMBERS
...
def _set_refund_payments(doc, pos, payments_data):
	...
	if abs(total) > abs(grand_total) + 0.01:
		frappe.throw(_("Refund payments {0} exceed the refund payable {1}").format(total, grand_total))
	if abs(total - grand_total) > 0.01:
		frappe.throw(_("Refund payments {0} must balance the refund total {1}").format(total, grand_total))
...
def _build_refund_response(doc, duplicate=False):
	...
	items_out = [
		{
			...
			"value_excluding_tax": flt(getattr(r, "custom_fbr_value_excluding_tax", 0) or 0, 2),
	...
	fbr_status = _cf("custom_fbr_status")
	fbr_invoice_number = _cf("custom_fbr_invoice_number")
	merchandise_refund = abs(flt(sum(flt(r.amount) for r in (doc.get("items") or [])), 2))
	gst_refund = abs(
		flt(
			sum(flt(getattr(r, "custom_fbr_sales_tax", 0) or 0) for r in (doc.get("items") or [])),
			2,
		)
	)
...
def submit_pos_refund(
	...
	if cint(offline_authenticated) and offline_auth_method != _OFFLINE_AUTH_METHOD_PIN:
		frappe.throw(
			_("Offline-authenticated refunds must use offline_auth_method '{0}'; got '{1}'").format(
				_OFFLINE_AUTH_METHOD_PIN, offline_auth_method
			),
			frappe.PermissionError,
		)
	...
	try:
		...
		frappe.db.set_value("POS Invoice", return_doc.name, "owner", cashier_user, update_modified=False)
