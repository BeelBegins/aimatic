...
def _site_pos_profile_name():
	row = frappe.db.get_value("POS Profile", {"disabled": 0}, "name", order_by="creation asc")
	return row


def _site_item_code():
	row = frappe.db.get_value("Item", {"disabled": 0, "is_sales_item": 1}, "item_code")
	return row
...
class TestAndroidBearerSecurity(_AimTestCase):
	...
	def test_public_oauth_client_id_is_resolved_by_app_name(self):
		...
		with patch(
			"aimatic.offline_pos.api.frappe.db.get_value", return_value="public-client-id"
		) as get_value:
			self.assertEqual(_get_pos_android_oauth_client_id(), "public-client-id")
			get_value.assert_called_once_with("OAuth Client", {"app_name": "Aimatic POS Android"}, "name")
...
class TestResolvePosCatalogItem(_AimTestCase):
	...
	def test_resolves_by_item_code(self):
		...
		self.assertEqual(
			result["price_list"], frappe.db.get_value("POS Profile", _POS_PROFILE_NAME, "selling_price_list")
		)
...
class TestPreviewCartFunctional(_AimTestCase):
	...
	def test_fbr_build_and_accounting_are_called(self):
		...
		with (
			patch("aimatic.fbr_pos.payload_builder.build_pos_payload", return_value={}) as mock_build,
			patch("aimatic.fbr_pos.accounting.apply_fbr_accounting_rows", return_value=None) as mock_acc,
		):
			preview_cart(
				self.pos_name,
				self.customer_name,
	...
	def test_fbr_tax_row_appears_in_top_level_taxes(self):
		...
		with (
			patch("aimatic.fbr_pos.payload_builder.build_pos_payload", return_value={}),
			patch("aimatic.fbr_pos.accounting.apply_fbr_accounting_rows", side_effect=_fake_accounting),
		):
			result = preview_cart(
				self.pos_name,
				self.customer_name,
	...
	def test_coupon_usage_count_not_updated(self):
		...
		with (
			_patch_fbr(),
			patch("erpnext.accounts.doctype.pricing_rule.utils.validate_coupon_code"),
			patch("erpnext.accounts.doctype.pricing_rule.utils.update_coupon_code_count") as mock_update,
		):
			preview_cart(
				self.pos_name,
				self.customer_name,
...
class TestPosRefund(_AimTestCase):
	...
	def test_refund_permission_allows_profile_user(self):
		...
		pos.meta = frappe._dict(has_field=lambda fieldname: fieldname == "custom_refund_allowed_users")
	...
	def test_returned_qty_aggregates_by_original_row(self):
		...
		with _patch("aimatic.offline_pos.api.returned_qty_by_row", return_value={"row-a": 2}):
			with self.assertRaises(frappe.ValidationError):
...
class TestStartPosSessionCashier(_AimTestCase):
	...
	def test_start_shift_as_cashier(self):
		...
		self.assertEqual(frappe.db.get_value("POS Opening Entry", result["name"], "user"), cashier)
...
class TestClosePosSessionCashier(_AimTestCase):
	...
	def test_close_shift_supervisor_allowed(self):
		...
		self.assertEqual(result["cashier_user"], cashier)
		self.assertEqual(frappe.db.get_value("POS Opening Entry", opening.name, "status"), "Closed")
	...
	def test_supervisor_cashier_can_close_through_plain_pos_user_terminal(self):
		...
		try:
			...
			self.assertFalse(frappe.has_permission("POS Closing Entry", ptype="create"))
			result = close_pos_session(opening.name, cashier, [])
		finally:
	...
	def test_plain_terminal_can_consolidate_shift_invoice_after_authorized_close(self):
		...
		with _patch_fbr(), patch("aimatic.fbr_pos.events.submit_pos_invoice_to_fbr", return_value=None):
	...
	def test_close_shift_normal_cashier_blocked(self):
		...
		self.assertEqual(frappe.db.get_value("POS Opening Entry", opening.name, "status"), "Open")
	...
	def test_close_shift_pos_user_with_valid_supervisor_token_allowed(self):
		...
		with _https_request():
			minted = authorize_pos_admin_action(supervisor, supervisor_password, "close_shift", "TEST-TERM-1")
...
class TestSubmitOnlineSaleCashier(_AimTestCase):
	...
	def test_idempotent_terminal_invoice_id_still_works(self):
		from aimatic.offline_pos.api import _build_pos_invoice_doc, submit_online_sale
		...
		if not frappe.db.exists(
			"POS Opening Entry",
			{"pos_profile": pos.name, "user": "Administrator", "status": "Open", "docstatus": 1},
		):
		...
		before_count = frappe.db.count("POS Invoice", {"custom_terminal_invoice_id": terminal_invoice_id})
		...
		after_count = frappe.db.count("POS Invoice", {"custom_terminal_invoice_id": terminal_invoice_id})
	...
	def test_offline_sale_cashier_pin_accepted(self):
		...
		self.assertEqual(frappe.db.get_value("POS Invoice", result["pos_invoice"], "owner"), cashier)
		self.assertEqual(
...
class _patch_fbr:
	...
	def __enter__(self):
		self._p1 = patch("aimatic.fbr_pos.payload_builder.build_pos_payload", return_value={})
		self._p2 = patch("aimatic.fbr_pos.accounting.apply_fbr_accounting_rows", return_value=None)
		self._p1.start()
		self._p2.start()
		return self
