...
ITEM_PRICE_BARCODE_FIELD = {
	...
	"read_only": 1,
	"description": (
		"All barcodes from the linked Item, in Item Barcode row order. Available in Item Price data exports."
	),
}
...
def backfill_item_price_barcodes():
	...
	for start in range(0, len(item_prices), BACKFILL_CHUNK_SIZE):
		...
		updates = {
			row.name: {"custom_barcodes": BARCODE_SEPARATOR.join(barcodes_by_item.get(row.item_code, []))}
