...
def is_fbr_optional_for_pos_profile(pos_profile):
	...
	return cint(frappe.db.get_value("POS Profile", pos_profile, "custom_fbr_optional"))
...
def resolve_fbr_settings(doc, company, branch):
	...
	if not settings_name:
		...
		frappe.throw(
			_("Enabled FBR Integration Settings not found for Company {0} and Branch {1}").format(
				frappe.bold(company),
				frappe.bold(branch),
			)
		)
...
def get_fbr_settings(company, branch):
	...
	if not api_url:
		frappe.throw(_("FBR POS API URL is missing for {0}").format(frappe.bold(environment)))
	...
	if submission_mode == "Real Time":
		...
		if not token:
			frappe.throw(_("FBR Security Token is required for Real Time mode"))
	else:
		token = settings.get_password(
			"security_token",
			raise_exception=False,
