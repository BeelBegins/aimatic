...
def add_fbr_sales_tax_row(doc, settings):
	...
	if not settings.get("sales_tax_account"):
		frappe.throw(_("Sales Tax Account is missing in FBR Integration Settings"))
...
def add_fbr_pos_fee_row(doc, settings):
	...
	if not settings.get("fbr_pos_fee_account"):
		frappe.throw(_("FBR POS Service Fee Account is missing in FBR Integration Settings"))
