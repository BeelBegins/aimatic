...
def get_invoice_branch(doc):
	...
	frappe.throw(_("Branch is missing on POS Invoice and POS Profile"))
...
def get_customer_snapshot(doc):
	...
	return {
		...
		"strn": (getattr(doc, "tax_strn", None) or customer_data.get("tax_strn") or ""),
		"ntn": (
		...
		"cnic": (getattr(doc, "tax_nic", None) or customer_data.get("tax_nic") or ""),
		"phone": (getattr(doc, "contact_mobile", None) or customer_data.get("mobile_no") or ""),
	}
...
def get_payment_mode(doc, settings):
	...
	mode = (primary_payment.mode_of_payment or "").strip().lower()
...
def get_item_hs_code(row, settings):
	...
	hs_code = clean_hs_code(item_hs_code) or clean_hs_code(settings.get("default_hs_code")) or DEFAULT_HS_CODE

	return hs_code


def get_discount_amount(row):
	return abs(money(getattr(row, "discount_amount", 0)))
...
def update_row_fbr_snapshot(row, config, calculated, hs_code):
	...
	if row.meta.has_field("custom_fbr_value_excluding_tax"):
		row.custom_fbr_value_excluding_tax = calculated["value_excluding_tax"]

	if row.meta.has_field("custom_fbr_sales_tax"):
		row.custom_fbr_sales_tax = calculated["sales_tax"]

	if row.meta.has_field("custom_fbr_retail_price"):
		row.custom_fbr_retail_price = calculated["retail_price"]
...
def build_item_payload(row, settings, invoice_type):
	...
	config = get_item_fbr_configuration(row.item_code)
	...
	sale_value = money(calculated["value_excluding_tax"])

	tax_charged = money(calculated["sales_tax"])
...
def build_return_item_payload(row):
	...
	if qty <= 0 or original_qty <= 0:
		frappe.throw(
			_("Quantity must be greater than zero for return item {0}").format(frappe.bold(row.item_code))
		)
...
def get_posting_datetime(doc):
	...
	return get_datetime(f"{doc.posting_date} {posting_time}")
...
def build_pos_payload(doc):
	...
	if is_return:
		...
		ref_usin = (getattr(original, "custom_fbr_invoice_number", None) if original else None) or getattr(
	...
	if not payload_items:
		frappe.throw(_("No valid items found for FBR payload"))

	total_sale_value = money(total_sale_value)
	total_tax_charged = money(total_tax_charged)
	total_discount = money(total_discount)

	total_bill_amount = money(total_sale_value + total_tax_charged - total_discount)
	...
	payload = {
		...
		"DateTime": posting_datetime.strftime("%Y-%m-%d %H:%M:%S.000"),
