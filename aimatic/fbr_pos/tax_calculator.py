...
def get_item_fbr_configuration(item_code):
	...
	if not item.custom_fbr_tax_category:
		frappe.throw(_("FBR Tax Category is missing for item {0}").format(frappe.bold(item_code)))
	...
	if not category:
		frappe.throw(
			_("FBR Tax Category {0} was not found").format(frappe.bold(item.custom_fbr_tax_category))
		)

	if not cint(category.enabled):
		frappe.throw(_("FBR Tax Category {0} is disabled").format(frappe.bold(item.custom_fbr_tax_category)))
	...
	if is_third_schedule and not cint(category.is_third_schedule):
		frappe.throw(
			_(
				"Item {0} is marked as Third Schedule, but selected FBR Tax Category is not Third Schedule"
			).format(frappe.bold(item_code))
		)

	if is_third_schedule and mrp <= 0:
		frappe.throw(_("MRP is required for Third Schedule item {0}").format(frappe.bold(item_code)))
...
def calculate_fbr_item(row, config):
	...
	if qty <= 0:
		frappe.throw(_("Quantity must be greater than zero for item {0}").format(frappe.bold(row.item_code)))
	...
	if is_third_schedule:
		...
		sales_tax = money(retail_price * tax_rate / (100 + tax_rate))
	...
	sales_tax = money(inclusive_value * tax_rate / (100 + tax_rate))
