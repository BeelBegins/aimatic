...
def _get_receipt_branch_context(doc):
	...
	company = frappe.db.get_value("Company", doc.company, ["tax_id", "phone_no", "email"], as_dict=True) or {}
	...
	phone = (
		(branch.get("custom_receipt_phone") if branch else None)
		or (address.get("phone") if address else None)
		or company.get("phone_no")
	)
	email = (
		(branch.get("custom_receipt_email") if branch else None)
		or (address.get("email_id") if address else None)
...
def get_pos_receipt_context(doc):
	...
	item_barcodes = {
		item.name: item.get("barcode") or master_barcodes.get(item.item_code) or "" for item in doc.items
	}
