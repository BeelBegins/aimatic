...
def _branch_warehouses(branch_filter: list[str] | None) -> list[str] | None:
	...
	return frappe.get_all(
		"Warehouse", filters={"custom_branch": ["in", branch_filter], "disabled": 0}, pluck="name"
	)
...
def get_sales_overview(
	...
	return {
		...
		"branch_breakdown": [
			{"branch": r.branch or _("Unassigned"), "net_sales": flt(r.net_sales)} for r in breakdown_rows
		],
	}
...
def rank_vendors(
	date_from: str | None = None, date_to: str | None = None, order: str = "best", limit: int = 10
) -> dict:
...
def get_inventory_vs_sales(
	direction: str = "overstock", lookback_days: int = 30, branch: str | None = None, limit: int = 20
) -> dict:
	...
	if not item_names:
		...
		if missing:
			for r in frappe.get_all(
				"Item", filters={"name": ["in", list(missing)]}, fields=["name", "item_name"]
			):
		...
		"branch_breakdown": [
			{
				...
				"returns_amount": flt(r.returns_amount),
				"returns_count": cint(r.returns_count),
			}
		...
		"top_suppliers": [
			{
				...
				"invoice_count": cint(r.voucher_count),
			}
			...
			"parameters": {
				...
				"properties": {
					...
					"order": {
						"type": "string",
						"enum": ["best", "worst"],
						"description": '"best" for top vendors by margin, "worst" for lowest margin / highest outstanding.',
					},
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					"direction": {
						"type": "string",
						"enum": ["overstock", "understock"],
						"description": '"overstock" for slow movers with too much stock relative to sales, "understock" for fast movers at risk of running out.',
					},
					"lookback_days": {
						"type": "integer",
						"description": "Trailing window in days to measure sales velocity, default 30, max 180.",
					},
					"branch": {
						"type": "string",
						"description": "Exact Branch name to scope to. Omit for all branches the user can see.",
			...
			"parameters": {
				...
				"properties": {
					...
					"order_by": {
						...
						"description": "Rank by sales revenue or by quantity sold, default revenue.",
					},
