...
from aimatic.ai.tools import _branch_warehouses, _get_date_range, _resolve_branch_filter, _resolve_company
...
_PURCHASE_DIMENSIONS = {
	"branch": "branch",
	"supplier": "supplier",
	"month": "DATE_FORMAT(posting_date, '%Y-%m-01')",
}
_GL_PURCHASE_DIMENSIONS = {
	"branch": "branch",
	"supplier": "party",
}  # outstanding_amount is a balance, not month-bucketable


def _query_purchase_measure(
	table: str, dimension: str | None, date_from, date_to, branch_filter, supplier: str | None
) -> dict[str, float]:
...
def _query_purchase_outstanding(
	dimension: str | None, date_to, branch_filter, supplier: str | None
) -> dict[str, float]:
	branch_clause = "AND branch IN %(branch_names)s" if branch_filter is not None else ""
...
def _query_purchases(
	measures: list[str], dimension: str | None, date_from, date_to, branch_filter, supplier: str | None
) -> list[dict]:
	if "outstanding_amount" in measures and dimension not in _GL_PURCHASE_DIMENSIONS:
		if dimension is not None:
			raise ValueError(
				f"outstanding_amount is a balance, not a flow - it can only be grouped by {sorted(_GL_PURCHASE_DIMENSIONS)}, not '{dimension}'."
			)

	per_measure: dict[str, dict[str, float]] = {}
	if "purchase_invoice_amount" in measures:
		per_measure["purchase_invoice_amount"] = _query_purchase_measure(
			"Purchase Invoice", dimension, date_from, date_to, branch_filter, supplier
		)
	if "goods_received_amount" in measures:
		per_measure["goods_received_amount"] = _query_purchase_measure(
			"Purchase Receipt", dimension, date_from, date_to, branch_filter, supplier
		)
	if "outstanding_amount" in measures:
		per_measure["outstanding_amount"] = _query_purchase_outstanding(
			dimension, date_to, branch_filter, supplier
		)
...
def _query_inventory(
	measures: list[str], dimension: str | None, branch_filter, warehouse_filter
) -> list[dict]:
	...
	stock_by_dim = {
		(r.dimension_value or _("Unassigned")): {
...
_PAYABLES_DIMENSIONS = {
	"supplier": "party",
	"branch": "branch",
	"aging_bucket": None,
}  # aging_bucket is Python-side
...
def _query_payables(dimension: str | None, date_to, branch_filter, supplier: str | None) -> list[dict]:
	...
	if dimension == "supplier":
		return [
			{"dimension_value": k, "outstanding_amount": round(v, 2)}
			for k, v in party_totals.items()
			if v > 0
		]
	if dimension == "branch":
...
def run_analytics_query(
	...
	if invalid_measures:
		return {
			"error": f"Invalid measure(s) for dataset '{dataset}': {invalid_measures}. Allowed: {sorted(_DATASET_MEASURES[dataset])}"
		}

	if dimension is not None and dimension not in _DATASET_DIMENSIONS[dataset]:
		return {
			"error": f"Invalid dimension for dataset '{dataset}': {dimension!r}. Allowed: {sorted(_DATASET_DIMENSIONS[dataset])}"
		}

	company = _resolve_company()
	currency = frappe.get_cached_value("Company", company, "default_currency")
	branch_filter = (
		_resolve_branch_filter(company, branch) if dataset in ("sales", "purchases", "payables") else None
	)
	warehouse_filter = (
		_branch_warehouses(_resolve_branch_filter(company, branch)) if dataset == "inventory" else None
	)
	...
	if compare_previous_period and dataset != "inventory":
		...
		for key in sorted(set(current_by_key) | set(previous_by_key), key=str):
			...
			for m in measures:
				...
				merged_row[f"{m}_change_pct"] = (
					round((cur_val - prev_val) / prev_val * 100, 2) if prev_val else None
				)
	...
	if sort and (sort in measures or sort == "dimension_value"):
		current_rows.sort(
			key=lambda r: (r.get(sort) if r.get(sort) is not None else 0), reverse=(sort_dir != "asc")
		)
			...
			"rows": [
				{
					...
					"customer": r.customer,
					"branch": r.branch or _("Unassigned"),
			...
			"parameters": {
				...
				"properties": {
					...
					"measures": {
						"type": "array",
						"items": {"type": "string"},
						"description": "One or more measure names valid for the chosen dataset.",
					},
					"dimension": {
						"type": "string",
						"description": "Optional dimension name valid for the chosen dataset, to group results by. Omit for a single overall total.",
					},
					"branch": {
						"type": "string",
					...
					"supplier": {
						"type": "string",
						"description": "Exact Supplier name to narrow to (purchases/payables datasets only).",
					},
					"date_from": {
						"type": "string",
						"description": "Start date, YYYY-MM-DD. Defaults to today if omitted. Not used by the inventory dataset (always current stock).",
					},
					"date_to": {
						"type": "string",
					...
					"sort": {
						"type": "string",
						"description": "A measure name (or 'dimension_value') to sort rows by.",
					},
					"sort_dir": {
						"type": "string",
						"enum": ["asc", "desc"],
					...
					"compare_previous_period": {
						"type": "boolean",
						"description": "If true, also computes the immediately preceding equal-length period and returns <measure>_previous and <measure>_change_pct per row. Not supported for the inventory dataset (current stock has no 'previous period').",
					},
				},
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					...
					"supplier": {
						"type": "string",
						"description": "Exact Supplier name (purchases/payables - required for payables).",
					},
					"customer": {
						"type": "string",
						"description": "Exact Customer name (sales dataset only).",
					},
					"item_code": {
						"type": "string",
						"description": "Exact Item code (inventory dataset only).",
					},
					"warehouse": {
						"type": "string",
						"description": "Exact Warehouse name (inventory dataset only).",
					},
					"date_from": {
						"type": "string",
