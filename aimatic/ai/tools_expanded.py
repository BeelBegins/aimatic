...
def get_sales_trend(
	...
	return {
		...
		"granularity": granularity,
		"trend": [
			{"bucket": str(r.bucket), "net_sales": flt(r.net_sales), "txn_count": cint(r.txn_count)}
			for r in rows
		],
	}
...
def get_hourly_sales_pattern(
	date_from: str | None = None, date_to: str | None = None, branch: str | None = None
) -> dict:
	...
	return {
		...
		"date_to": str(date_to),
		"by_hour": [
			{"hour": cint(r.hour), "net_sales": flt(r.net_sales), "txn_count": cint(r.txn_count)}
			for r in hour_rows
		],
		"by_weekday": [
			{
				"weekday": _WEEKDAY_NAMES[cint(r.weekday)],
				"net_sales": flt(r.net_sales),
				"txn_count": cint(r.txn_count),
			}
...
def get_discount_overview(
	...
	return {
		...
		"effective_discount_pct": round(total_discount / (total_discount + total_net_sales) * 100, 2)
		if (total_discount + total_net_sales)
		else 0,
...
def get_sales_by_item_group(
	date_from: str | None = None, date_to: str | None = None, branch: str | None = None, limit: int = 20
) -> dict:
...
def get_purchase_concentration(
	date_from: str | None = None, date_to: str | None = None, branch: str | None = None, top_n: int = 5
) -> dict:
	...
	top_suppliers = [
		{"supplier": s, "amount": amt, "share_pct": round(amt / total_spend * 100, 2) if total_spend else 0}
		for s, amt in ranked[:top_n]
	]
...
def get_stock_aging(branch: str | None = None, limit: int = 30) -> dict:
	...
	results.sort(
		key=lambda r: (r["days_since_last_receipt"] is not None, -(r["days_since_last_receipt"] or 0))
	)
	return {"company": company, "currency": currency, "items": results[:limit]}


def get_reorder_recommendations(
	branch: str | None = None, lookback_days: int = 30, lead_time_days: int = 7, limit: int = 20
) -> dict:
	...
	for r in sales_rows:
		...
		item_name = (
			stock.item_name
			if stock
			else (frappe.get_cached_value("Item", r.item_code, "item_name") or r.item_code)
		)
	...
	stale_shifts = [
		{
			...
			"user": o.user,
			"opened_at": str(o.period_start_date),
			"days_open": (today_date - getdate(o.period_start_date)).days,
		}
		for o in openings
	]
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					...
					"limit": {
						"type": "integer",
						"description": "Max item groups to return, default 20, max 50.",
					},
				},
			},
		},
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					...
					"branch": {
						"type": "string",
						"description": "Exact Branch name to scope selling price lists to. Omit for all branches the user can see.",
					},
				},
			},
		},
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					"item_code": {
						"type": "string",
						"description": "Exact Item code to compare across suppliers.",
					},
					"months": {
						"type": "integer",
						"description": "Lookback window in months, default 6, max 24.",
					},
					"branch": {
						"type": "string",
						"description": "Exact Branch name to scope purchase history to. Omit for all branches the user can see.",
					},
				},
				"required": ["item_code"],
			},
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					"date_from": {
						"type": "string",
						"description": "Start date (PO transaction_date), YYYY-MM-DD. Defaults to today if omitted.",
					},
					"date_to": {
						"type": "string",
						"description": "End date, YYYY-MM-DD. Defaults to date_from if omitted.",
					...
					"supplier": {
						"type": "string",
						"description": "Exact Supplier name to narrow to one vendor. Omit for all suppliers.",
					},
					"branch": {
						"type": "string",
						"description": "Exact Branch name to scope to. Omit for all branches the user can see.",
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					...
					"branch": {
						"type": "string",
						"description": "Exact Branch name to scope purchases to. Omit for all branches the user can see.",
					},
					"top_n": {
						"type": "integer",
						"description": "How many top suppliers to include, default 5, max 30.",
					},
				},
			},
		},
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					...
					"lookback_days": {
						"type": "integer",
						"description": "Window to measure sales velocity, default 30, max 180.",
					},
					"lead_time_days": {
						"type": "integer",
						"description": "Assumed supplier lead time in days, default 7, max 60.",
					},
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					"lookback_days": {
						"type": "integer",
						"description": "Segmentation window in days, default 90, max 365.",
					},
					"branch": {
						"type": "string",
						"description": "Exact Branch name to scope to. Omit for all branches the user can see.",
		...
		"function": {
			...
			"parameters": {
				...
				"properties": {
					"stale_days": {
						"type": "integer",
						"description": "Age threshold in days to flag as stale, default 7, max 90.",
					},
					"branch": {
						"type": "string",
						"description": "Exact Branch name to scope draft documents and open shifts to. Omit for all branches the user can see.",
					},
				},
			},
		},
