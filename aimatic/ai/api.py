...
from aimatic.ai.abc_xyz_analysis import TOOL_DISPATCH as _ABC_XYZ_DISPATCH
from aimatic.ai.abc_xyz_analysis import TOOL_SPECS as _ABC_XYZ_SPECS
from aimatic.ai.advanced_intelligence_tools import TOOL_DISPATCH as _ADVANCED_BI_DISPATCH
from aimatic.ai.advanced_intelligence_tools import TOOL_SPECS as _ADVANCED_BI_SPECS
from aimatic.ai.analytics_engine import TOOL_DISPATCH as _ANALYTICS_DISPATCH
from aimatic.ai.analytics_engine import TOOL_SPECS as _ANALYTICS_SPECS
from aimatic.ai.demand_forecasting import TOOL_DISPATCH as _FORECAST_DISPATCH
from aimatic.ai.demand_forecasting import TOOL_SPECS as _FORECAST_SPECS
from aimatic.ai.dynamic_report import DYNAMIC_REPORT_DISPATCH
from aimatic.ai.dynamic_report import TOOL_SPECS as _DYNAMIC_REPORT_SPECS
...
from aimatic.ai.price_recommendation import TOOL_DISPATCH as _PRICE_DISPATCH
from aimatic.ai.price_recommendation import TOOL_SPECS as _PRICE_SPECS
from aimatic.ai.report_runner import TOOL_DISPATCH as _REPORT_RUNNER_DISPATCH
from aimatic.ai.report_runner import TOOL_SPECS as _REPORT_RUNNER_SPECS
...
from aimatic.ai.tools import TOOL_DISPATCH as _CORE_DISPATCH
from aimatic.ai.tools import TOOL_SPECS as _CORE_SPECS
from aimatic.ai.tools import _resolve_branch_filter, _resolve_company
from aimatic.ai.tools_accounts import TOOL_DISPATCH as _ACCOUNTS_DISPATCH
from aimatic.ai.tools_accounts import TOOL_SPECS as _ACCOUNTS_SPECS
from aimatic.ai.tools_expanded import TOOL_DISPATCH as _EXPANDED_DISPATCH
from aimatic.ai.tools_expanded import TOOL_SPECS as _EXPANDED_SPECS
from aimatic.ai.tools_extended import TOOL_DISPATCH as _EXTENDED_DISPATCH
from aimatic.ai.tools_extended import TOOL_SPECS as _EXTENDED_SPECS
...
TOOL_SPECS = (
	...
	+ _ACCOUNTS_SPECS
	+ _EXPANDED_SPECS
	+ _REPORT_RUNNER_SPECS
	+ _ANALYTICS_SPECS
	+ _DYNAMIC_REPORT_SPECS
	+ _ABC_XYZ_SPECS
	+ _FORECAST_SPECS
	+ _PRICE_SPECS
	+ _ADVANCED_BI_SPECS
...
TOOL_DISPATCH = {
	...
	**_ACCOUNTS_DISPATCH,
	**_EXPANDED_DISPATCH,
	**_REPORT_RUNNER_DISPATCH,
	**_ANALYTICS_DISPATCH,
	**DYNAMIC_REPORT_DISPATCH,
	**_ABC_XYZ_DISPATCH,
	**_FORECAST_DISPATCH,
	**_PRICE_DISPATCH,
	**_ADVANCED_BI_DISPATCH,
}
...
_PREFERRED_FREE_AUDIO_MODELS = ("nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free",)
...
def _get_free_audio_model(force_refresh=False) -> str:
	...
	if response.status_code != 200:
		raise NemotronError(f"OpenRouter model catalogue returned {response.status_code}: {response.text}")
...
def _build_system_prompt() -> str:
	...
	company = (
		frappe.defaults.get_user_default("Company") or frappe.defaults.get_default("company") or "the company"
	)
	return (
...
def ask(message: str, history: str | None = None, conversation: str | None = None):
	...
	frappe.throw(
		_(
			"The current certified analytics could not answer this question reliably. No business figures were produced."
		)
	)
...
def _log_turn(role: str, content: str, conversation: str | None = None):
	...
	try:
		doc = frappe.get_doc(
			{"doctype": "AI Assistant Message", "user": frappe.session.user, "role": role, "content": content}
		)
		if conversation:
...
def pin_conversation(conversation: str, pinned):
	...
	_check_conversation_ownership(conversation)
	frappe.db.set_value(
		"AI Assistant Conversation", conversation, "pinned", 1 if cint(pinned) else 0, update_modified=True
	)
	return {"ok": True}
...
def submit_feedback(message: str, feedback: str, note: str | None = None):
	...
	frappe.db.set_value(
		"AI Assistant Message",
		message,
		{"feedback": feedback, "feedback_note": note or ""},
...
def list_dashboards():
	...
	return {
		"dashboards": [
			{"name": r.name, "title": r.title, "widget_count": count_map.get(r.name, 0)} for r in rows
		]
	}
