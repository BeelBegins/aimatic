...
SIMPLE_QUESTIONS = [
	_case("simple", f"What were net sales {period}?", "get_sales_overview", ["date_from", "date_to"], "kpi")
	for period in _PERIODS
] + [
	_case("simple", f"Show top selling items at {branch}.", "get_top_selling_items", ["branch", "date_from", "date_to"], "table")
	for branch in _BRANCHES
] + [
	_case("simple", question, tool, parameters, output)
	for question, tool, parameters, output in [
		("How much did customers return this month?", "get_returns_overview", ["date_from", "date_to"], "kpi"),
		("Show sales by item group this month.", "get_sales_by_item_group", ["date_from", "date_to"], "chart"),
		...
		("Show this month's expense breakdown.", "get_expense_breakdown", ["date_from", "date_to"], "chart"),
...
COMPARISON_QUESTIONS = [
	...
	for question, tool, parameters in [
		("Compare returns this week versus last week.", "get_returns_overview", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare gross margin this month versus last month.", "get_gross_margin_overview", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare purchases this quarter to the prior quarter.", "get_purchase_overview", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare discounts this month with last month.", "get_discount_overview", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare hourly sales for this Friday and last Friday.", "get_hourly_sales_pattern", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare branch profitability this month and last month.", "get_branch_profit_and_loss", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare expense categories this quarter versus last quarter.", "get_expense_breakdown", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare item-group sales this month and previous month.", "get_sales_by_item_group", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare supplier purchase concentration year over year.", "get_purchase_concentration", ["date_from", "date_to", "comparison_from", "comparison_to"]),
		("Compare receivables aging with last month end.", "get_receivables_aging", ["date_to", "comparison_to"]),
		("Compare transaction count this week and last week.", "get_sales_overview", ["date_from", "date_to", "comparison_from", "comparison_to"]),
	]
]

DIAGNOSTIC_QUESTIONS = [
	_case("diagnostic", f"Why did sales change at {branch}?", "get_business_anomalies", ["branch", "lookback_days"], "warning_table")
	for branch in _BRANCHES
] + [
	...
	for question, tool, parameters, output in [
		...
		("Find sudden supplier-price increases.", "get_business_anomalies", ["lookback_days"], "warning_table"),
		("Show suspicious transaction-count patterns.", "get_business_anomalies", ["lookback_days"], "warning_table"),
		("Why are returns high this month?", "get_returns_overview", ["date_from", "date_to"], "driver_table"),
		("Which branches caused the margin decline?", "get_branch_profit_and_loss", ["date_from", "date_to"], "driver_table"),
		...
		("Which items are frequently bought together?", "get_market_basket_analysis", ["history_days", "minimum_transactions"], "table"),
		("Was the selected promotion effective?", "get_promotion_effectiveness", ["item_code", "promotion_from", "promotion_to"], "kpi"),
...
FORECASTING_QUESTIONS = [
	...
	for question, parameters, insufficient in [
		("Forecast beverages by week for eight weeks.", ["item_group", "granularity", "forecast_horizon"], False),
...
INVENTORY_QUESTIONS = [
	...
	for question, tool, parameters, output in [
		...
		("Show C-class items holding excessive stock.", "get_abc_xyz_analysis", ["date_from", "date_to"], "table"),
		("Show high-value irregular-demand items.", "get_abc_xyz_analysis", ["date_from", "date_to"], "table"),
		("Classify by gross-margin contribution.", "get_abc_xyz_analysis", ["abc_metric", "date_from", "date_to"], "matrix"),
		("Classify by quantity sold.", "get_abc_xyz_analysis", ["abc_metric", "date_from", "date_to"], "matrix"),
		("Classify stock value with 70/90 thresholds.", "get_abc_xyz_analysis", ["abc_metric", "abc_a_threshold", "abc_b_threshold"], "matrix"),
		("Recommend branch transfers.", "get_branch_transfer_recommendations", ["history_days", "target_cover_days"], "table"),
		("Which branch can transfer ITEM-001?", "get_branch_transfer_recommendations", ["item_code"], "table"),
		("Transfer excess beverage stock between branches.", "get_branch_transfer_recommendations", ["item_group"], "table"),
		("Show transfer recommendations above ten units.", "get_branch_transfer_recommendations", ["minimum_transfer_qty"], "table"),
		...
		("Show the stock-value versus contribution scatter.", "get_abc_xyz_analysis", ["date_from", "date_to"], "scatter"),
		("Find items with insufficient XYZ history.", "get_abc_xyz_analysis", ["date_from", "date_to"], "table"),
		("Analyze one warehouse's ABC/XYZ mix.", "get_abc_xyz_analysis", ["warehouse", "date_from", "date_to"], "matrix"),
	]
]
...
PRICING_QUESTIONS = [
	...
	for question, parameters, insufficient in [
		...
		("Protect market share without violating margin.", ["item_code", "objective", "minimum_margin_pct"], False),
		("Maintain volume with no more than 5% price movement.", ["item_code", "objective", "maximum_price_change_pct"], False),
		("Compare conservative, recommended, and aggressive price scenarios.", ["item_code", "include_scenarios"], False),
