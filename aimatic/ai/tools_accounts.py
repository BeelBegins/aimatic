...
from aimatic.ai.tools import _resolve_company, _get_date_range
...
def get_payables_aging(limit: int = 10) -> dict:
    ...
    for row in gl_entries:
        ...
        supplier_buckets[party]["outstanding_amount"] = flt(supplier_buckets[party]["outstanding_amount"]) + amount
        supplier_buckets[party][f"bucket_{bucket_key.replace('-', '_').replace('+', '_plus')}"] = (
            flt(supplier_buckets[party][f"bucket_{bucket_key.replace('-', '_').replace('+', '_plus')}"]) + amount
        )
...
def get_receivables_aging(limit: int = 10) -> dict:
    ...
    for row in gl_entries:
        ...
        customer_buckets[party]["outstanding_amount"] = flt(customer_buckets[party]["outstanding_amount"]) + amount
        customer_buckets[party][f"bucket_{bucket_key.replace('-', '_').replace('+', '_plus')}"] = (
            flt(customer_buckets[party][f"bucket_{bucket_key.replace('-', '_').replace('+', '_plus')}"]) + amount
        )
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "date_from": {"type": "string", "description": "Start date, YYYY-MM-DD. Defaults to date_to if omitted - resolve phrases like \"this month\" or \"this year\" into a concrete date yourself before calling."},
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "date_from": {"type": "string", "description": "Start date, YYYY-MM-DD. Defaults to date_to if omitted - resolve phrases like \"this month\" or \"this year\" into a concrete date yourself before calling."},
                    "date_to": {"type": "string", "description": "End date (YYYY-MM-DD), defaults to today."},
                    "limit": {"type": "integer", "description": "Max accounts to return, default 10, max 30."},
                },
            },
        },
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "date_from": {"type": "string", "description": "Start date for Income/Expense, YYYY-MM-DD. Defaults to date_to if omitted - resolve phrases like \"this month\" or \"this year\" into a concrete date yourself before calling."},
                    "date_to": {"type": "string", "description": "End date for all (YYYY-MM-DD), defaults to today."},
                },
            },
        },
    },
