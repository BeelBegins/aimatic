...
def get_item_price_history(item_code: str, months: int = 12) -> dict:
    ...
    return {
        ...
        "fast_snapshot": {
            ...
            "latest_price_source_date": str(item_doc.custom_latest_price_source_date) if item_doc and item_doc.custom_latest_price_source_date else None,
        },
    }


def get_price_increases(months: int = 3, min_change_pct: float = 10.0, branch: str | None = None, limit: int = 20) -> dict:
    ...
    if branch_filter is not None and not branch_filter:
        return {"company": company, "currency": currency, "date_from": str(date_from), "date_to": str(date_to), "items": []}
...
def get_dead_stock_detail(min_days_since_last_sale: int = 180, branch: str | None = None, limit: int = 30) -> dict:
    ...
    if warehouse_filter is not None and not warehouse_filter:
        return {"company": company, "currency": currency, "min_days_since_last_sale": min_days_since_last_sale, "items": [], "total_dead_stock_value": 0}
...
def get_top_customers(date_from: str | None = None, date_to: str | None = None, branch: str | None = None, order_by: str = "revenue", limit: int = 10) -> dict:
    ...
    if branch_filter is not None and not branch_filter:
        return {"company": company, "currency": currency, "date_from": str(date_from), "date_to": str(date_to), "order_by": order_by, "customers": []}
    ...
    params = {"company": company, "date_from": date_from, "date_to": date_to, "branch_names": tuple(branch_filter) if branch_filter else (), "limit": limit}
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "months": {"type": "integer", "description": "Lookback window in months for purchase history, default 12, max 24."},
                },
                "required": ["item_code"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "months": {"type": "integer", "description": "Lookback window in months, default 3, max 24."},
                    "min_change_pct": {"type": "number", "description": "Minimum % increase to include, default 10.0."},
                    "branch": {"type": "string", "description": "Exact Branch name to scope to (filters PR/PI header branch). Omit for all branches the user can see."},
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "min_days_since_last_sale": {"type": "integer", "description": "Minimum days since last sale to qualify as dead stock, default 180."},
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "branch": {"type": "string", "description": "Exact Branch name to scope to. Omit for all branches the user can see."},
                    "order_by": {"type": "string", "enum": ["revenue", "frequency"], "description": "Rank by sales revenue or by transaction count, default revenue."},
                    "limit": {"type": "integer", "description": "Max customers to return, default 10, max 50."},
                },
            },
        },
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "limit": {"type": "integer", "description": "Max customers to return, default 10, max 30."},
                },
            },
        },
    },
