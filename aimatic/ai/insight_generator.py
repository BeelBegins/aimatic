...
def _detect_dead_stock(tool_results: dict[str, dict]) -> list[Insight]:
	...
	items = res.get("items", [])
	dead_items = [
		it for it in items if it.get("sales_qty_in_window", 0) == 0 and it.get("stock_value", 0) > 0
	]
	if not dead_items:
		return []
...
def _detect_stockout_risk(tool_results: dict[str, dict]) -> list[Insight]:
	...
	items = res.get("items", [])
	at_risk = [it for it in items if it.get("days_of_stock") is not None and it["days_of_stock"] < 7]
	if not at_risk:
		return []
				...
				"top_items": [
					{
						"item_code": it["item_code"],
						"item_name": it["item_name"],
						"days_of_stock": it["days_of_stock"],
...
def _detect_negative_vendor_margin(tool_results: dict[str, dict]) -> list[Insight]:
	...
	weak_vendors = [
		v for v in vendors if v.get("gross_margin_pct") is not None and v["gross_margin_pct"] < 10
	]
	if not weak_vendors:
		return []
		...
		Insight(
			...
			supporting_data={
				...
				"vendors": [
					{
						"supplier": v["supplier"],
						"gross_margin_pct": v["gross_margin_pct"],
...
def _detect_branch_underperformance(tool_results: dict[str, dict]) -> list[Insight]:
	...
	underperformers = [b for b in branches if b.get("net_sales", 0) < threshold]
				...
				"underperformers": [
					{
						...
						"pct_of_mean": round(b["net_sales"] / mean_sales * 100, 1),
					}
