...
def _kpis_for_get_payables_aging(result: dict) -> list[KPI]:
	...
	overdue = (
		flt(result.get("buckets", {}).get("31-60", 0))
		+ flt(result.get("buckets", {}).get("61-90", 0))
		+ flt(result.get("buckets", {}).get("90+", 0))
	)
	if overdue:
		kpis.append(
			KPI(
...
def _table_for_get_payables_aging(result: dict) -> Table | None:
	...
	return Table(
		...
		columns=[
			...
			TableColumn(
				key="outstanding_amount", label="Total Outstanding", type="currency", currency=currency
			),
...
def _kpis_for_get_profit_and_loss_overview(result: dict) -> list[KPI]:
	...
	for key, label, severity in [
		...
		(
			"net_margin_pct",
			"Net Margin %",
			"warning" if flt(result.get("net_margin_pct", 0)) < 10 else "info",
		),
	]:
		val = result.get(key)
		if val is not None:
...
def _kpis_for_get_open_documents_overview(result: dict) -> list[KPI]:
	...
	kpis = [
		KPI(
			key="get_open_documents_overview_stale_docs",
			label="Stale Draft Documents",
			value=float(total_stale),
			format="number",
			severity="warning" if total_stale else None,
		),
		KPI(
			key="get_open_documents_overview_stale_shifts",
			label="Stale Open Shifts",
			value=float(len(stale_shifts)),
			format="number",
			severity="warning" if stale_shifts else None,
		),
	]
	return kpis
...
def _table_for_get_outstanding_payables_overview(result: dict) -> Table | None:
	...
	return Table(
		...
		columns=[
			TableColumn(key="supplier", label="Supplier", type="link", doctype="Supplier"),
			TableColumn(
				key="outstanding_amount", label="Outstanding Amount", type="currency", currency=currency
			),
...
def _table_for_get_item_price_history(result: dict) -> Table | None:
	...
	return Table(
		...
		columns=[
			...
			TableColumn(
				key="price_after_taxes", label="Price After Taxes", type="currency", currency=currency
			),
		],
		rows=history,
		sortable=True,
...
def _table_for_run_dynamic_report(result: dict) -> Table | None:
	...
	for key in rows[0].keys():
		if key in date_fields:
			...
			columns.append(
				TableColumn(key=key, label=key.replace("_", " ").title(), type="currency", currency=None)
			)
		elif link_fields.get(key):
			columns.append(
				TableColumn(
					key=key, label=key.replace("_", " ").title(), type="link", doctype=link_fields[key]
				)
			)
		else:
...
def _table_for_run_frappe_report(result: dict) -> Table | None:
	...
	for c in columns_meta:
		...
		if coltype == "link":
			columns.append(
				TableColumn(
					key=fieldname, label=c.get("label") or fieldname, type="link", doctype=c.get("options")
				)
			)
		elif coltype == "currency":
			columns.append(
				TableColumn(
					key=fieldname, label=c.get("label") or fieldname, type="currency", currency=currency
				)
			)
		else:
...
def _table_for_run_analytics_query(result: dict) -> Table | None:
	...
	columns = [
		TableColumn(
			key="dimension_value",
			label=(result.get("dimension") or "Group").replace("_", " ").title(),
			type="text",
		)
	]
	for key in rows[0].keys():
...
def build_response(
	...
	purchase_tools = {
		...
		"get_po_receipt_variance",
		"get_supplier_price_comparison",
	}
	customer_tools = {
		"get_top_customers",
		"get_receivables_overview",
		"get_customer_activity_segments",
	}
