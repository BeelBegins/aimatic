...
def _chart_get_stock_aging(result: dict) -> Chart | None:
	items = [row for row in (result.get("items") or []) if row.get("days_since_last_receipt") is not None][
		:_MAX_CHART_BARS
	]
	if len(items) < 2:
		return None
...
def _chart_run_analytics_query(result: dict) -> Chart | None:
	...
	fmt = (
		"percent"
		if "pct" in first_measure
		else ("currency" if any(t in first_measure for t in ("amount", "sales", "value")) else "number")
	)
	return Chart(
		...
		type="bar",
		data=ChartData(
			labels=labels, datasets=[{"label": first_measure.replace("_", " ").title(), "data": data}]
		),
		options=ChartOptions(horizontal=True, yAxis={"format": fmt}),
		auto_selected=True,
	)
