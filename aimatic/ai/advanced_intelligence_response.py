...
from aimatic.ai.response_schema import KPI, Chart, ChartData, ChartOptions, Pagination, Table, TableColumn
...
def _transfer_kpis(result):
	...
	return (
		[
			...
			KPI(
				"transfer_quantity", "Transfer Quantity", sum(flt(r.get("transfer_qty")) for r in rows), "qty"
			),
			KPI(
				"transfer_dead_stock_reduction",
				"Expected Dead-Stock Reduction",
				sum(flt(r.get("expected_dead_stock_reduction")) for r in rows),
				"currency",
			),
		]
		if rows
...
def _promotion_kpis(result):
	kpis = [
		...
		KPI(
			"promo_incremental_revenue",
			"Incremental Revenue",
			flt(result.get("incremental_revenue")),
			"currency",
		),
	]
	if result.get("incremental_margin") is not None:
		kpis.append(
			KPI(
				...
				"Incremental Margin",
				flt(result.get("incremental_margin")),
				"currency",
			)
		)
	...
	return (
		[
			...
			KPI(
				...
				len(result.get("high_value_customers") or []),
				"number",
			),
			KPI(
				"rfm_dormant",
				"Dormant Customers",
				len(result.get("dormant_customers") or []),
				"number",
				severity="warning",
			),
		]
	...
	return (
		[
			...
			KPI(
				...
				"Purchase Volume",
				sum(flt(r.get("purchase_volume")) for r in rows),
				"currency",
			),
			KPI(
				"vendor_avg_reliability",
				"Average Reliability",
				sum(flt(r.get("reliability_score")) for r in rows) / len(rows),
				"percent",
			),
		]
		if rows
...
def _anomaly_kpis(result):
	...
	return [
		...
		KPI(
			"anomaly_critical",
			"Critical Anomalies",
			sum(r.get("severity") == "critical" for r in rows),
			"number",
			severity="critical",
		),
	]
...
def _transfer_table(result):
	return _table(
		"table_branch_transfers",
		"Branch Transfer Recommendations",
		_rows(result, "recommendations"),
		[
...
def _vendor_table(result):
	return _table(
		"table_vendor_reliability",
		"Vendor Reliability",
		_rows(result, "vendors"),
		[
...
def _anomaly_table(result):
	return _table(
		"table_business_anomalies",
		"Business Anomalies",
		_rows(result, "anomalies"),
		[
...
def _bar_chart(chart_id, title, labels, data, label, fmt="number"):
	...
	return [
		Chart(
			...
			title,
			"bar",
			ChartData(labels, [{"label": label, "data": data}]),
			ChartOptions(horizontal=True, yAxis={"format": fmt}),
			True,
		)
	]
...
def _transfer_charts(r):
	...
	return _bar_chart(
		...
		[f"{x.get('from_branch')} → {x.get('to_branch')}" for x in rows],
		[flt(x.get("transfer_qty")) for x in rows],
		"Quantity",
		"qty",
	)
...
def _promotion_charts(r):
	return _bar_chart(
		"chart_promotion",
		"Baseline vs Promotion",
		["Baseline", "Promotion"],
		[flt(r.get("baseline_sales")), flt(r.get("promotional_sales"))],
		"Sales",
		"currency",
	)
	...
	return (
		[
			Chart(
				...
				"Customer Segments",
				"donut",
				ChartData(list(counts), [{"label": "Customers", "data": list(counts.values())}]),
				ChartOptions(),
				True,
			)
		]
...
def _basket_charts(r):
	...
	return _bar_chart(
		"chart_baskets",
		"Strongest Basket Lift",
		[f"{x.get('item_a')} + {x.get('item_b')}" for x in rows],
		[flt(x.get("lift")) for x in rows],
		"Lift",
	)
...
def _vendor_charts(r):
	...
	return _bar_chart(
		"chart_vendors",
		"Vendor Reliability",
		[x.get("supplier") for x in rows],
		[flt(x.get("reliability_score")) for x in rows],
		"Reliability",
		"percent",
	)
...
def _anomaly_charts(r):
	...
	return _bar_chart(
		"chart_anomalies",
		"Anomaly Magnitude",
		[f"{x.get('branch') or x.get('warehouse')} — {x.get('metric')}" for x in rows],
		[abs(flt(x.get("z_score") or x.get("variance"))) for x in rows],
		"Magnitude",
	)
