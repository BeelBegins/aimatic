...
_ALLOWED_DOCTYPES: dict[str, DoctypeConfig] = {
    "POS Invoice": DoctypeConfig(
        fields=frozenset({"name", "posting_date", "customer", "branch", "pos_profile", "grand_total", "is_return", "docstatus"}),
    ...
    "Purchase Invoice": DoctypeConfig(
        fields=frozenset({"name", "posting_date", "supplier", "branch", "base_grand_total", "outstanding_amount", "is_return", "docstatus"}),
        date_field="posting_date",
        company_field="company",
        branch_field="branch",
        mandatory_filters={"docstatus": 1},
    ...
    "Purchase Receipt": DoctypeConfig(
        fields=frozenset({"name", "posting_date", "supplier", "branch", "base_grand_total", "is_return", "docstatus"}),
        date_field="posting_date",
        company_field="company",
        branch_field="branch",
        mandatory_filters={"docstatus": 1},
...
def run_dynamic_report(
    ...
    if config is None:
        return {"error": f"DocType not available for dynamic reporting: {doctype}. Available: {sorted(_ALLOWED_DOCTYPES)}"}
    ...
    for f in requested_fields:
        if f not in config.fields:
            return {"error": f"Field '{f}' is not available for dynamic reporting on {doctype}. Allowed: {sorted(config.fields)}"}

    if group_by and group_by not in config.groupable:
        return {"error": f"Cannot group {doctype} by '{group_by}'. Allowed group-by fields: {sorted(config.groupable)}"}

    if order_by and order_by not in config.fields and order_by != "value":
        return {"error": f"Cannot order {doctype} by '{order_by}'. Allowed: {sorted(config.fields)} or 'value' when aggregating."}
    ...
    if aggregate_field is not None or aggregate_fn is not None:
        if aggregate_field not in config.aggregatable:
            return {"error": f"Cannot aggregate {doctype}.{aggregate_field}. Allowed: {sorted(config.aggregatable)}"}
    ...
    if config.branch_field:
        ...
        if branch_filter is not None and not branch_filter:
            ...
            return {"doctype": doctype, "row_count": 0, "rows": [], "filters_applied": {}, "date_from": None, "date_to": None, "limit": limit}
...
TOOL_SPECS = [
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "doctype": {"type": "string", "enum": sorted(_ALLOWED_DOCTYPES), "description": "The DocType to query."},
                    "fields": {"type": "array", "items": {"type": "string"}, "description": "Plain fields to return (ignored if aggregate_field/aggregate_fn are given)."},
                    "aggregate_field": {"type": "string", "description": "Field to aggregate, e.g. grand_total."},
                    "aggregate_fn": {"type": "string", "enum": sorted(ALLOWED_AGGREGATIONS), "description": "Aggregation function to apply to aggregate_field."},
                    "filters": {"type": "object", "description": "Simple equality filters, e.g. {\"supplier\": \"ABC Traders\"}. Range/comparison filters aren't supported yet."},
                    "group_by": {"type": "string", "description": "Field to group by when aggregating."},
                    "order_by": {"type": "string", "description": "Field to sort by (or 'value' when aggregating)."},
                    "order_dir": {"type": "string", "enum": ["asc", "desc"], "description": "Sort direction, default desc."},
                    "date_from": {"type": "string", "description": "Start date YYYY-MM-DD, for DocTypes with a date field. Defaults to 90 days ago if omitted."},
