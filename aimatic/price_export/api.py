...
def _normalize_foodpanda_price_updates(updates):
	...
	if len(updates) > _MAX_GRID_PRICE_UPDATES:
		raise ValueError(f"A maximum of {_MAX_GRID_PRICE_UPDATES} Foodpanda prices can be saved at once.")
...
def _extract_branch_price_sheet_updates(sheet):
	...
	for row_number, values in enumerate(sheet.iter_rows(min_row=1, max_row=50, values_only=True), start=1):
		headers = [_normalize_branch_sheet_header(value) for value in values]
		if "item code" not in headers:
			continue
		matching_price_headers = [header for header in _BRANCH_SHEET_PRICE_HEADERS if header in headers]
	...
	if duplicates:
		raise ValueError(
			"Duplicate Item Codes are not allowed in the workbook: " + ", ".join(sorted(duplicates)[:20])
		)
	if len(updates) > _MAX_GRID_PRICE_UPDATES:
		raise ValueError(f"A maximum of {_MAX_GRID_PRICE_UPDATES} Foodpanda prices can be imported at once.")
	return updates, {
