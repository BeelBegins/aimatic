...
def _next_sibling_group_number(parent_account, parent_number, default_block=100):
	...
	sibling_numbers = frappe.get_all(
		"Account", filters={"parent_account": grandparent}, pluck="account_number"
	)
	bigger_siblings = sorted(
		n for n in (_as_int(v) for v in sibling_numbers) if n is not None and n > parent_number
	)
