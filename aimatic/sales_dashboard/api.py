...
@frappe.whitelist()
def get_dashboard_summary(company: str | None = None, date_from: str | None = None, date_to: str | None = None, branch: str | None = None):
...
def get_pos_profile_detail(pos_profile: str, date: str | None = None):
    ...
    profile_meta = frappe.db.get_value("POS Profile", pos_profile, ["name", "company", "branch"], as_dict=True)
    if not profile_meta:
    ...
    return {
        ...
        "payment_split": _get_payment_split(profile_meta.company, target_date, target_date, branch_filter=None, pos_profile=pos_profile),
...
@frappe.whitelist()
def get_returns_detail(company: str | None = None, date_from: str | None = None, date_to: str | None = None, branch: str | None = None):
    company, currency = _resolve_context(company, branch)
    date_from, date_to = _get_date_range(date_from, date_to)
    branch_filter = _resolve_branch_filter(company, branch)
...
@frappe.whitelist()
def get_payment_mode_detail(company: str | None = None, date_from: str | None = None, date_to: str | None = None, branch: str | None = None):
    company, currency = _resolve_context(company, branch)
    date_from, date_to = _get_date_range(date_from, date_to)
    branch_filter = _resolve_branch_filter(company, branch)
    return {"currency": currency, "payment_split": _get_payment_split(company, date_from, date_to, branch_filter=branch_filter)}
...
def _get_range_kpis(company: str, date_from, date_to, branch_filter: list[str] | None):
    if branch_filter is not None and not branch_filter:
        return {"net_sales": 0, "gross_sales": 0, "returns_amount": 0, "returns_count": 0, "txn_count": 0, "average_basket": 0}
...
def _get_today_branch_profile_grid(company: str, branch_filter: list[str] | None):
    ...
    sections = [_build_branch_section(name, profiles, totals_by_profile, shift_by_profile) for name in branch_names_from_profiles]
...
def _get_open_shifts(company: str, branch_filter: list[str] | None):
    ...
    full_names = {
        u.name: u.full_name for u in frappe.get_all("User", filters={"name": ["in", user_names]}, fields=["name", "full_name"])
    }
...
def _get_hourly_breakdown(pos_profile: str, target_date):
    ...
    return [{"hour": cint(row.hour), "amount": flt(row.amount), "txn_count": cint(row.txn_count)} for row in rows]


def _get_payment_split(company: str, date_from, date_to, branch_filter: list[str] | None = None, pos_profile: str | None = None):
    if branch_filter is not None and not branch_filter:
        return []

    conditions = ["pi.docstatus = 1", "pi.company = %(company)s", "pi.posting_date BETWEEN %(date_from)s AND %(date_to)s"]
