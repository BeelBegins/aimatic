...
def _ensure_warehouse(company):
	...
	if existing:
		return existing
	parent = _leaf_value("Warehouse", {"company": company, "is_group": 1, "parent_warehouse": ["is", "not set"]})
	if not parent:
...
def _ensure_item(spec, company, warehouse, price_list, item_group):
	...
	for uom, rate in (("Pcs", spec["piece_rate"]), ("Box", spec["piece_rate"] * spec["box_factor"] * 0.95)):
		...
		if price:
			frappe.db.set_value("Item Price", price, {"price_list_rate": rate, "valid_from": add_days(nowdate(), -90)})
		else:
			frappe.get_doc({
				"doctype": "Item Price",
				"item_code": code,
...
def _ensure_delivery_location(customer, address, index):
	name = frappe.db.get_value("Mobile Sales Delivery Location", {"customer": customer, "address": address}, "name")
	if name:
		return name
	return frappe.get_doc({
...
def _ensure_assortment(customer, item=None, item_group=None):
	filters = {"customer": customer, "item": item or ["is", "not set"], "item_group": item_group or ["is", "not set"]}
...
def _ensure_discount_authority(user):
	...
	if name:
		frappe.db.set_value("Mobile Sales Discount Authority", name, {"enabled": 1, "maximum_discount_percent": 5})
		return name
	return frappe.get_doc({
...
def _ensure_order(reference, company, branch, warehouse, price_list, customer, items, transaction_date, discount=0, submit=False):
	...
	doc = frappe.get_doc({
		...
		"items": [{**row, "warehouse": warehouse, "delivery_date": add_days(transaction_date, 7)} for row in items],
	})
...
def _ensure_pending_approval(order, requested_by):
	...
	if name:
		frappe.db.set_value("Mobile Sales Discount Approval", name, {"status": "Pending", "requested_by": requested_by})
		return name
...
def _ensure_visit(company, warehouse, customer, address, assigned_to, route_order):
	filters = {"customer": customer, "assigned_to": assigned_to, "scheduled_date": nowdate(), "instructions": ["like", f"{DEMO_PREFIX}%"]}
...
def execute(company=None, assigned_to=None):
	...
	currency = frappe.db.get_value("Price List", price_list, "currency") or frappe.db.get_value("Company", company, "default_currency")
	...
	item_specs = [
		{"item_code": "AIM-DEMO-DRILL", "item_name": "AIMATIC Demo Cordless Drill", "brand": brands[0], "box_factor": 6, "piece_rate": 1800, "stock": 120, "valuation": 1100},
		{"item_code": "AIM-DEMO-GLOVES", "item_name": "AIMATIC Demo Safety Gloves", "brand": brands[1], "box_factor": 12, "piece_rate": 250, "stock": 240, "valuation": 125},
		{"item_code": "AIM-DEMO-TAPE", "item_name": "AIMATIC Demo Measuring Tape", "brand": brands[0], "box_factor": 10, "piece_rate": 600, "stock": 80, "valuation": 350},
		{"item_code": "AIM-DEMO-OOS", "item_name": "AIMATIC Demo Out of Stock Item", "brand": brands[1], "box_factor": 4, "piece_rate": 900, "stock": 0, "valuation": 500},
	]
	items = []
	stock_entries = []
	for spec in item_specs:
		...
		if spec["stock"]:
			entry = _ensure_stock(spec["item_code"], warehouse, company, branch, spec["stock"], spec["valuation"])
			if entry:
	...
	delivery_locations = [_ensure_delivery_location(customer, addresses[index], index + 1) for index, customer in enumerate(customers[:2])]
	assortments = [
	...
	order_items = [{"item_code": items[0], "qty": 1, "uom": "Box"}, {"item_code": items[1], "qty": 2, "uom": "Box"}]
	editable = _ensure_order("AIMATIC-DEMO-EDIT", company, branch, warehouse, price_list, customers[0], order_items, nowdate(), discount=4)
	pending = _ensure_order("AIMATIC-DEMO-APPROVAL", company, branch, warehouse, price_list, customers[1], order_items, nowdate(), discount=12)
	approval = _ensure_pending_approval(pending, assigned_to)
	history_one = _ensure_order("AIMATIC-DEMO-HISTORY-1", company, branch, warehouse, price_list, customers[0], [{"item_code": items[0], "qty": 1, "uom": "Box"}], add_days(nowdate(), -14), submit=True)
	history_two = _ensure_order("AIMATIC-DEMO-HISTORY-2", company, branch, warehouse, price_list, customers[0], [{"item_code": items[0], "qty": 2, "uom": "Box"}], add_days(nowdate(), -7), submit=True)
	history_three = _ensure_order("AIMATIC-DEMO-HISTORY-3", company, branch, warehouse, price_list, customers[0], [{"item_code": items[0], "qty": 1, "uom": "Box"}], nowdate(), submit=True)
	history_four = _ensure_order("AIMATIC-DEMO-HISTORY-4", company, branch, warehouse, price_list, customers[0], [{"item_code": items[0], "qty": 3, "uom": "Box"}], nowdate(), submit=True)
	cancel_candidate = _ensure_order("AIMATIC-DEMO-CANCEL", company, branch, warehouse, price_list, customers[3], [{"item_code": items[2], "qty": 2, "uom": "Pcs"}], nowdate(), submit=True)
	cancelled_sample = _ensure_order("AIMATIC-DEMO-CANCELLED", company, branch, warehouse, price_list, customers[3], [{"item_code": items[2], "qty": 1, "uom": "Pcs"}], nowdate(), submit=True)
	...
	visits = [_ensure_visit(company, warehouse, customer, addresses[index], assigned_to, index + 1) for index, customer in enumerate(customers[:2])]
	frappe.db.commit()
	return {
		"company": company,
		"warehouse": warehouse,
		...
		"promotion": promotion,
		"orders": [editable.name, pending.name, history_one.name, history_two.name, history_three.name, history_four.name, cancel_candidate.name, cancelled_sample.name],
