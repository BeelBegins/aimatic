...
aimatic.SalesDashboardPage = class SalesDashboardPage {
	...
	get_date_range() {
		...
		if (preset === __("Custom")) {
			return [
				this.date_from_field.get_value() || todayStr,
				this.date_to_field.get_value() || todayStr,
			];
		}
	...
	async refresh(opts) {
		...
		try {
			const data = await frappe.xcall(
				"aimatic.sales_dashboard.api.get_dashboard_summary",
				filters
			);
	...
	render_error(error) {
		const message = frappe.utils.escape_html(
			error && error.message ? error.message : __("Unable to load the sales dashboard.")
		);
		this.$container.html(`
		...
		this.$container.html(`
            ...
            <div class="sd-grid">
                ${this.card(
					...
					"trending-up",
					__("Net Sales"),
					this.money(kpis.net_sales),
					`${__("Selected range")}: ${frappe.datetime.str_to_user(
						data.date_from
                ...
                ${this.card(
					...
					"shopping-bag",
					__("Transactions"),
					this.number(kpis.txn_count),
					`${__("Avg. basket")}: ${this.money(kpis.average_basket)}`
				)}
                ${this.card(
					"purchase",
					"credit-card",
					__("Average Basket"),
					this.money(kpis.average_basket),
					__("Per sale, selected range")
				)}
                ${this.card(
					"payable",
					"rotate-ccw",
					__("Returns"),
					this.money(kpis.returns_amount),
					`${this.number(kpis.returns_count)} ${__("return(s)")}`,
					undefined,
					"returns"
				)}
                ${this.card(
					...
					"clock",
					__("Active Shifts"),
					this.number(kpis.active_shift_count),
					__("Currently open, live totals"),
					undefined,
					"active_shifts"
				)}
                ${this.card(
					...
					"award",
					__("Top Branch"),
					kpis.top_branch || __("No sales yet"),
					kpis.top_branch ? this.money(kpis.top_branch_amount) : "",
					undefined,
					"top_branch"
				)}
            </div>
			...
			data.trend_days || 14,
                ...
                <div class="sd-chart-card">
                    <h4>${__("Branch Comparison")} <span class="sd-item-meta">${__(
			"Selected range"
		)}</span></h4>
	...
	render_payment_mix(paymentSplit) {
			...
			.map((row) => {
				...
				return `
                    <div class="sd-payment-row">
                        <div class="sd-payment-label">${frappe.utils.escape_html(
							row.mode_of_payment || __("Unknown")
						)}</div>
                        <div class="sd-payment-bar-wrap">
                            <div class="sd-payment-bar" style="width:${pct.toFixed(1)}%"></div>
                        </div>
                        <div class="sd-payment-value">${this.money(
							row.amount
						)} <span class="sd-item-meta">(${pct.toFixed(0)}%)</span></div>
                    </div>
                `;
			})
			.join("");
		return `
            <div class="sd-payment-mix">
                <h4>${__("Payment Mix")} <span class="sd-item-meta">${__(
			"Selected range"
		)}</span></h4>
                ${bars}
            </div>
	...
	render_branch_chart(branchComparison) {
		...
		if (!branchComparison.length) {
			$(el).html(
				`<div class="sd-empty sd-empty-inline">${__(
					"No branches with sales in this window yet."
				)}</div>`
			);
			return;
		}
		new frappe.Chart(el, {
			data: {
				...
				datasets: [
					{
						name: __("Net Sales"),
						values: branchComparison.map((r) => flt(r.net_sales)),
					},
				],
			},
			type: "bar",
		...
		if (!branches.length) {
			return `
                <div class="sd-empty">
                    ...
                    ${__(
						"No branches or enabled POS Profiles are configured for this company yet."
					)}
                </div>
            `;
		}
	...
	render_branch_section(section) {
		const profilesHtml = section.profiles.length
			...
			: `<div class="sd-empty sd-empty-inline">${__(
					"No enabled POS Profiles for this branch."
			  )}</div>`;
		return `
            <div class="sd-branch-section">
                <div class="sd-branch-heading">
                    <h3>${frappe.utils.escape_html(section.branch)}</h3>
                    <div class="sd-branch-total">${this.money(
						section.today_total
					)} <span class="sd-item-meta">${__("today")}</span></div>
                </div>
	...
	render_profile_card(profile) {
		...
		const statusBadge = shift
			? `<span class="sd-badge sd-badge-open">${__("Open")} · ${frappe.utils.escape_html(
					shift.cashier_full_name
			  )} · ${__("since")} ${frappe.datetime.str_to_user(shift.opened_at)}</span>`
			: `<span class="sd-badge sd-badge-closed">${__("Closed")}</span>`;
		const lastSale = profile.last_sale_at
			? comment_when(profile.last_sale_at)
			: __("No sales yet today");
		const shiftLine = shift
			? `<div class="sd-profile-shift-line">${__("Current shift")}: <strong>${this.money(
					shift.running_total
			  )}</strong></div>`
			: "";
		return `
            <div class="sd-profile-card" data-sd-profile="${frappe.utils.escape_html(
				profile.pos_profile
			)}">
                <div class="sd-profile-top">
                    <div class="sd-profile-name">${frappe.utils.escape_html(
						profile.pos_profile
					)}</div>
                </div>
                <div class="sd-profile-value">${this.money(profile.today_sales)}</div>
                <div class="sd-profile-meta">${this.number(profile.txn_count)} ${__(
			"transactions"
		)} · ${lastSale}</div>
                ${shiftLine}
	...
	open_dialog(title) {
		...
		dialog.$body.html(`
            <div class="sd-loading-inline">${frappe.utils.icon("loader", "md")} ${__(
			"Loading..."
		)}</div>
        `);
		dialog.show();
	...
	async open_kpi_detail(key) {
		...
		if (key === "returns") {
			...
			try {
				const data = await frappe.xcall(
					"aimatic.sales_dashboard.api.get_returns_detail",
					filters
				);
				this.currency = data.currency;
		...
		if (key === "active_shifts") {
			...
			try {
				const data = await frappe.xcall(
					"aimatic.sales_dashboard.api.get_active_shifts_detail",
					{
	...
	async open_profile_detail(posProfile) {
		...
		try {
			const data = await frappe.xcall("aimatic.sales_dashboard.api.get_pos_profile_detail", {
	...
	render_dialog_error(dialog, error) {
		console.error(error);
		const message = frappe.utils.escape_html(
			error && error.message ? error.message : __("Unable to load this detail.")
		);
	...
	render_returns_table(rows) {
		if (!rows.length) {
			return `<div class="sd-empty sd-empty-inline">${__(
				"No returns in the selected range."
			)}</div>`;
		}
		const body = rows
			.map(
            ...
            <tr>
                ...
                <td>${
					row.return_against ? this.docLink("POS Invoice", row.return_against) : ""
				}</td>
		...
		return `
            <div class="sd-grid sd-drill-summary">
                ${this.card(
					"revenue",
					"trending-up",
					data.is_today ? __("Today So Far") : __("Selected Day"),
					this.money(today.amount),
					`${this.number(today.txn_count)} ${__("transactions")}`
				)}
                ${this.card(
					"purchase",
					"rotate-ccw",
					__("Same Time Yesterday"),
					this.money(yesterday.amount),
					`${this.number(yesterday.txn_count)} ${__("transactions")}`
				)}
	...
	render_profile_hourly_chart(dialog, hourly) {
		...
		if (!hourly.length) {
			$(el).html(
				`<div class="sd-empty sd-empty-inline">${__("No sales recorded yet.")}</div>`
			);
			return;
		}
		const byHour = {};
	...
	render_recent_invoices_table(rows) {
		...
		const body = rows
            ...
            <tr class="${row.is_return ? "sd-row-return" : ""}">
                <td>${this.docLink("POS Invoice", row.name)}</td>
                <td>${frappe.datetime.str_to_user(row.posting_date)} ${frappe.utils
					.escape_html(row.posting_time || "")
					.slice(0, 5)}</td>
                <td>${frappe.utils.escape_html(row.customer || "")}</td>
	...
	update_last_updated() {
		this.$container
			.find("[data-sd-last-updated]")
			.text(
				`${__("Last updated")}: ${
					frappe.datetime.get_time
						? frappe.datetime.get_time(frappe.datetime.now_datetime())
