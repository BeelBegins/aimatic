...
aimatic.AI_ASSISTANT_KPI_BUTTONS = [
	...
	{
		label: __("Branch Comparison"),
		question: "Compare net sales across all branches this month.",
	},
	{
		label: __("Payment Mode Split"),
		question: "What's the payment mode breakdown for this month?",
	},
	{ label: __("Returns Today"), question: "How many returns and how much refunded today?" },
	{
		label: __("Active Shifts"),
		question: "What POS shifts are currently open and what are their running totals?",
	},
	...
	{
		label: __("Outstanding Payables"),
		question: "Which vendors do I owe the most money to right now?",
	},
	{ label: __("Purchases This Month"), question: "What were my purchases this month?" },
	{
		label: __("Top Selling Items"),
		question: "What are my top selling items this month by revenue?",
	},
	{
		label: __("Overstocked Items"),
		question: "Which items are overstocked relative to their sales?",
	},
	{
		label: __("Understocked Items"),
		question: "Which items are at risk of running out of stock?",
	},
...
aimatic.AiAssistantPage = class AiAssistantPage {
	...
	apply_enabled_state(enabled) {
		...
		if (!enabled) {
			$(
				`<div class="ai-assistant-disabled-banner">${frappe.utils.icon(
					"alert-triangle",
					"xs"
				)} ${__("AI Assistant is currently disabled by an administrator.")}</div>`
	...
	open_settings_dialog() {
		frappe
			.call({ method: "aimatic.ai.api.get_ai_integration_settings" })
			.then((settings_r) => {
				frappe
					.call({ method: "aimatic.ai.api.list_available_free_models" })
					.then((models_r) => {
						const settings = settings_r.message || {};
						const models = models_r.message || [];
						const options = models.map((m) => ({
							value: m.id,
							label: `${m.name} — ${Math.round(
								(m.context_length || 0) / 1000
							)}K context (free)`,
						}));
						...
						const dialog = new frappe.ui.Dialog({
							...
							fields: [
								...
								},
								{
									...
									default:
										settings.model || (options[0] && options[0].value) || "",
									description: __(
										"Only currently free, tool-calling-capable OpenRouter models are listed."
									),
								},
								{
									...
									options: `<p class="text-muted small">${__(
										"The OpenRouter API key itself is configured on the server and is not shown or editable here."
									)}</p>`,
								},
							],
							...
							options.forEach((o) =>
								$select.append(
									`<option value="${frappe.utils.escape_html(
										o.value
									)}">${frappe.utils.escape_html(o.label)}</option>`
								)
							);
	...
	build_layout() {
		this.$layout = $(`
            <div class="ai-assistant-layout">
                <div class="ai-assistant-sidebar-rail">
                    <button class="btn-reset ai-assistant-sidebar-collapse" title="${__(
						"Collapse"
					)}">${frappe.utils.icon("chevrons-left", "sm")}</button>
                </div>
                <div class="ai-assistant-sidebar">
                    <div class="ai-assistant-sidebar-header">
                        <button class="btn btn-sm btn-primary ai-assistant-new-btn">${frappe.utils.icon(
							"plus",
							"xs"
						)} ${__("New Analysis")}</button>
                    </div>
                    <input type="text" class="form-control input-xs ai-assistant-conv-search" placeholder="${__(
                    ...
                    <div class="ai-assistant-dashboard-section">
                        <div class="ai-assistant-dashboard-header">
                            ...
                            <button class="btn-reset ai-assistant-new-dashboard-btn" title="${__(
								"New Dashboard"
							)}">${frappe.utils.icon("plus", "xs")}</button>
                        </div>
                        <div class="ai-assistant-dashboard-list"></div>
                    </div>
                </div>
                <div class="ai-assistant-container">
                    <div class="ai-assistant-executive-shortcuts" aria-label="${__(
						"Executive dashboards"
					)}"></div>
                    <div class="ai-assistant-intro">
                        ${__(
							"Ask about sales, purchases, vendors, inventory, or customers. Rich answers now include KPIs, charts, tables, and insights. Use the context bar to scope your question."
						)}
                    </div>
                    ...
                    <div class="ai-assistant-input-row">
                        <button class="btn btn-default ai-assistant-mic-btn" title="${__(
							"Record a voice question"
						)}">${frappe.utils.icon("mic", "sm")}</button>
                ...
                <div class="ai-assistant-dashboard-view" style="display:none;">
                    <div class="ai-assistant-dashboard-view-header">
                        <button class="btn btn-sm btn-default ai-assistant-back-to-chat">${frappe.utils.icon(
							"chevrons-left",
							"xs"
						)} ${__("Back to Chat")}</button>
                        <h4 class="ai-assistant-dashboard-view-title"></h4>
                        <button class="btn btn-sm btn-primary ai-assistant-refresh-dashboard" title="${__(
							"Refresh every widget with current data"
						)}" disabled>
                            ${frappe.utils.icon("refresh", "xs")} <span>${__(
			"Refresh Dashboard"
		)}</span>
                        </button>
                    </div>
                ...
                <div class="ai-assistant-right-panel">
                    ...
                    <div class="ai-assistant-right-panel-body">
                        <div class="small text-muted">${__(
							"Ask a question to see its data sources and scope here."
						)}</div>
                    </div>
                </div>
                <div class="ai-assistant-right-panel-rail">
                    <button class="btn-reset ai-assistant-right-panel-collapse" title="${__(
						"Collapse"
					)}">${frappe.utils.icon("panel-right", "sm")}</button>
                </div>
            </div>
		...
		this.$backdrop = $('<div class="ai-assistant-drawer-backdrop"></div>').appendTo(
			this.$layout
		);
		...
		this.$context_bar_fields = $(
			'<div class="ai-assistant-context-bar-fields"></div>'
		).appendTo(this.$context_bar);
		this.$context_bar_toggle = $(
			'<button type="button" class="ai-assistant-context-bar-toggle"></button>'
		).prependTo(this.$context_bar);
	...
	build_sidebar_events() {
		...
		this.$layout.find(".ai-assistant-sidebar-collapse").on("click", (e) => {
			...
			$btn.html(
				collapsed
					? frappe.utils.icon("chevrons-right", "sm")
		...
		this.$layout.find(".ai-assistant-right-panel-collapse").on("click", (e) => {
			...
			$btn.html(
				collapsed
					? frappe.utils.icon("panel-left", "sm")
					: frappe.utils.icon("panel-right", "sm")
			);
		...
		this.$conv_search.on("input", () =>
			this.filter_conversation_list(this.$conv_search.val())
		);

		// Phase 3: Dashboard section events
		this.$layout
			.find(".ai-assistant-new-dashboard-btn")
			.on("click", () => this.create_dashboard());
		this.$layout
			.find(".ai-assistant-back-to-chat")
			.on("click", () => this.hide_dashboard_view());
	...
	maybe_toggle_backdrop($panel, collapsed) {
		...
		if (collapsed) {
			const other_open =
				!this.$sidebar.hasClass("collapsed") || !this.$right_panel.hasClass("collapsed");
	...
	sync_rail_icons() {
		...
		this.$layout
			.find(".ai-assistant-sidebar-collapse")
		...
		this.$layout
			.find(".ai-assistant-right-panel-collapse")
	...
	update_context_bar_toggle() {
		...
		this.$context_bar_toggle.html(
			(expanded
				? frappe.utils.icon("chevron-up", "xs")
	...
	save_current_answer(response, question) {
		const defaultTitle =
			response.answer && response.answer.summary
	...
	prompt_add_to_dashboard(savedReportName) {
		frappe.call({ method: "aimatic.ai.api.list_dashboards" }).then((r) => {
			...
			const chosen = prompt(
				__("Add to dashboard? Type dashboard title (available: {0})", [titles])
			);
			if (!chosen || !chosen.trim()) return;

			const match = dashboards.find(
				(d) => d.title.toLowerCase() === chosen.trim().toLowerCase()
			);
			if (!match) {
	...
	render_dashboard_list(dashboards) {
		...
		if (!dashboards.length) {
			this.$dashboard_list.append(
				`<div class="small text-muted p-2">${__(
		...
		dashboards.forEach((db) => {
			const $row = $(`
                <div class="ai-assistant-dashboard-row" data-name="${frappe.utils.escape_html(
					db.name
				)}">
                    <span class="ai-assistant-dashboard-title">${frappe.utils.escape_html(
						db.title
					)}</span>
                    <span class="ai-assistant-dashboard-count small text-muted">(${
						db.widget_count || 0
					})</span>
                </div>
            `);
	...
	render_executive_shortcuts(dashboards) {
		...
		const featured = aimatic.AI_ASSISTANT_EXECUTIVE_DASHBOARDS.map((definition) => ({
			definition,
		...
		featured.forEach(({ definition, dashboard }) => {
			const $button = $(
				`<button type="button" class="btn btn-default ai-assistant-executive-shortcut">
                    <span class="ai-assistant-executive-icon">${frappe.utils.icon(
						definition.icon,
						"sm"
					)}</span>
                    ...
                    <span class="ai-assistant-executive-open">${frappe.utils.icon(
						"chevron-right",
						"xs"
					)}</span>
                </button>`
	...
	open_dashboard(name) {
		...
		return frappe
			.call({ method: "aimatic.ai.api.get_dashboard", args: { name: name } })
			.then((r) => {
	...
	confirm_refresh_dashboard() {
		...
		if (!reportNames.length) {
			frappe.show_alert({
				message: __("This dashboard has no widgets to refresh"),
				indicator: "orange",
			});
			return;
		}
		...
		frappe.confirm(
			__(
				"Refresh {0} dashboard widgets with current data? This uses one or more OpenRouter requests per widget and may take a few minutes.",
				[reportNames.length]
			),
		...
		try {
			...
			if (!reloadedDashboard) {
				frappe.show_alert(
					{
						message: __(
							"Widget data was refreshed, but the dashboard could not reload. Please reopen it."
						),
						indicator: "orange",
					},
					7
			...
			frappe.show_alert(
				{
					message: skipped
						? __(
								"Dashboard refreshed: {0} updated, {1} kept their previous snapshot.",
								[refreshed, skipped]
						  )
	...
	render_dashboard_view(dashboard) {
		...
		if (!widgets.length) {
			this.$dashboard_grid.append(
				`<div class="small text-muted p-3">${__(
					"No widgets yet - save an answer from the chat and add it to this dashboard."
				)}</div>`
			);
			return;
		}
		...
		const sizeClass = {
			Small: "ai-assistant-widget-sm",
			Medium: "ai-assistant-widget-md",
			Large: "ai-assistant-widget-lg",
		};
		const pendingCharts = [];
		...
		widgets.forEach((widget) => {
			...
			const $card = $(`
                <div class="ai-assistant-dashboard-widget ${
					sizeClass[widget.size] || "ai-assistant-widget-md"
				}">
                    <div class="ai-assistant-widget-header">
                        <span class="ai-assistant-widget-title">${frappe.utils.escape_html(
							widget.title || ""
						)}</span>
                        <button class="btn-reset ai-assistant-widget-remove" title="${__(
	...
	render_conversation_list(conversations) {
		...
		if (!conversations.length) {
			this.$conv_list.append(
				`<div class="ai-assistant-conv-empty small text-muted">${__(
		...
		conversations.forEach((conv) => {
			const meta_html = conv.last_activity
				? `<div class="ai-assistant-conv-meta">${this.relative_time(
						conv.last_activity
				  )}</div>`
				: "";
			const $row = $(`
                <div class="ai-assistant-conv-row ${
					conv.name === this.current_conversation ? "active" : ""
				}" data-name="${frappe.utils.escape_html(conv.name)}">
                    <div class="ai-assistant-conv-row-top">
                        <span class="ai-assistant-conv-pin-icon">${
							conv.pinned ? frappe.utils.icon("pin", "xs") : ""
						}</span>
                        <span class="ai-assistant-conv-title">${frappe.utils.escape_html(
							conv.title
						)}</span>
                        <span class="ai-assistant-conv-actions">
                            <button class="btn-reset ai-assistant-conv-pin" title="${__(
								"Pin"
							)}">${frappe.utils.icon("pin", "xs")}</button>
                            <button class="btn-reset ai-assistant-conv-rename" title="${__(
								"Rename"
							)}">${frappe.utils.icon("pencil", "xs")}</button>
                            <button class="btn-reset ai-assistant-conv-delete" title="${__(
								"Delete"
			...
			$row.find(".ai-assistant-conv-pin").on("click", (e) => {
				...
				frappe
					.call({
						method: "aimatic.ai.api.pin_conversation",
						args: { conversation: conv.name, pinned: conv.pinned ? 0 : 1 },
					})
					.then(() => this.refresh_conversation_list());
			});
			$row.find(".ai-assistant-conv-rename").on("click", (e) => {
				...
				if (new_title && new_title.trim()) {
					frappe
						.call({
							method: "aimatic.ai.api.rename_conversation",
							args: { conversation: conv.name, title: new_title.trim() },
						})
						.then(() => this.refresh_conversation_list());
				}
			});
			$row.find(".ai-assistant-conv-delete").on("click", (e) => {
				...
				frappe.confirm(__("Delete this conversation? This cannot be undone."), () => {
					frappe
						.call({
							method: "aimatic.ai.api.delete_conversation",
		...
		frappe
			...
			.then((r) => {
				...
				if (has_more && messages.length) {
					...
					const $btn = $(
						`<button type="button" class="btn btn-xs btn-default ai-assistant-load-older-btn">${__(
							"Load older messages"
						)}</button>`
					);
					$btn.on("click", () => {
	...
	update_right_panel(response) {
		...
		const sources_html = sources.length
			? sources
					.map(
						(s) =>
							`<div class="ai-assistant-rp-source"><span class="ai-assistant-rp-source-type">${frappe.utils.escape_html(
								s.type
							)}</span> ${frappe.utils.escape_html(s.name)}</div>`
					)
					.join("")
			: `<div class="small text-muted">${__(
					"No specific data source (general knowledge answer)."
			  )}</div>`;
		...
		this.$right_panel.find(".ai-assistant-right-panel-body").html(`
            <div class="ai-assistant-rp-section">
                ...
                <div class="ai-assistant-rp-value">${frappe.utils.escape_html(
					ctx.data_freshness || ""
				)}</div>
            </div>
            <div class="ai-assistant-rp-section">
	...
	build_suggested_questions() {
		...
		const $list = $('<div class="ai-assistant-suggested-list" hidden></div>').appendTo(
			$container
		);

		questions.forEach((q) => {
			$(
				`<button class="btn btn-default btn-xs ai-assistant-suggested-btn">${frappe.utils.escape_html(
					q
				)}</button>`
			)
				.on("click", () => {
	...
	_toggle_suggested($header, $list) {
		...
		$header
			.find(".ai-assistant-suggested-chevron")
			.html(frappe.utils.icon(isExpanded ? "chevron-down" : "chevron-up", "xs"));
	}
	...
	init_voice() {
		...
		if (
			...
			this.$mic_btn
				.prop("disabled", true)
				.attr("title", __("Audio recording is not supported in this browser."));
	...
	render_voice_status() {
		...
		this.$mic_btn
			...
			.html(
				isRecording
					? frappe.utils.icon("circle-stop", "sm")
					: frappe.utils.icon("mic", "sm")
			);
		...
		if (isRecording) {
			...
			this.$voice_status.html(`
                ...
                <span class="ai-assistant-voice-bars">
                    ${[0, 1, 2, 3, 4]
						.map(() => '<span class="ai-assistant-voice-bar"></span>')
						.join("")}
                </span>
                <span class="small text-muted">${__(
					"Recording — tap Stop to transcribe (maximum 45 seconds)."
				)}</span>
            `);
			...
			this.$voice_status.html(`
                ...
                <span class="small text-muted">${__(
					"Transcribing with a free OpenRouter model…"
				)}</span>
            `);
			...
			this.$voice_status.html(`
                <span class="small text-muted">${__(
					"Transcript ready — edit if needed, then Send."
				)}</span>
                ${
					model
						? `<span class="small ai-assistant-voice-model" title="${model}">${__(
								"Free model"
						  )}</span>`
						: ""
				}
                <span class="small text-muted">${__(
					"Language is auto-detected; Urdu is best-effort on the current free model."
				)}</span>
			...
			this.$voice_status.html(
				`<span class="small ai-assistant-voice-error">${__(
					"Microphone permission denied. Allow microphone access in your browser to use voice input."
				)}</span>`
			);
		} else if (this.voice_state === "network_error") {
			this.$voice_status.html(
				`<span class="small ai-assistant-voice-error">${__(
					"Transcription needs a network connection. Try again or type your question."
				)}</span>`
			);
			...
			this.$voice_status.html(
				`<span class="small ai-assistant-voice-error">${__(
					"Free OpenRouter transcription failed. No paid fallback was used. Try again or type your question."
				)}${detail ? ` ${detail}` : ""}</span>`
			);
	...
	clear() {
		...
		this.$right_panel &&
			this.$right_panel
				.find(".ai-assistant-right-panel-body")
				.html(
					`<div class="small text-muted">${__(
						"Ask a question to see its data sources and scope here."
					)}</div>`
	...
	send_message(rawMessage) {
		if (this.voice_state === "recording") {
			this.stop_recording();
			frappe.show_alert({
				message: __("Wait for the transcript, review it, then tap Send."),
				indicator: "blue",
			});
			return;
		}
		if (this.voice_state === "processing") {
			frappe.show_alert({
				message: __("Transcription is still in progress."),
				indicator: "blue",
			});
			return;
		}
	...
	build_bubble(role, content, is_error) {
		const $bubble = $(`
            <div class="ai-assistant-bubble ai-assistant-bubble-${role}${
			is_error ? " ai-assistant-bubble-error" : ""
		}">
                <div class="ai-assistant-bubble-role">${
					role === "user" ? __("You") : __("Assistant")
				}</div>
	...
	render_rich_answer(response, question) {
		...
		const $turn = $(`<div class="ai-assistant-rich-turn"></div>`).attr(
			"data-question",
		...
		if (response.answer && (response.answer.executive_summary || response.answer.summary)) {
			$turn.append(
				this.render_summary(response.answer.executive_summary || response.answer.summary)
			);
		}
		...
		const $saveBtn = $(
			`<button class="btn btn-xs btn-default ai-assistant-save-report" title="${__(
				"Save this answer as a report"
			)}">${frappe.utils.icon("save", "xs")} ${__("Save")}</button>`
		);
	...
	render_key_drivers(drivers) {
		...
		wrap.append(
			jQuery('<div class="ai-assistant-section-title"></div>').text(__("Key Drivers"))
		);
		const list = jQuery('<div class="ai-assistant-driver-list"></div>');
		drivers.forEach((driver) => {
			const item = jQuery('<div class="ai-assistant-driver-item"></div>');
			item.append(
				jQuery('<span class="ai-assistant-driver-label"></span>').text(driver.label || "")
			);
			item.append(
				jQuery('<span class="ai-assistant-driver-value"></span>').text(
					format_number(driver.value || 0, null, 2) +
						(driver.contribution_pct != null
							? " (" + format_number(driver.contribution_pct, null, 2) + "%)"
							: "")
				)
			);
			list.append(item);
	...
	render_recommendations(recommendations) {
		const wrap = jQuery('<div class="ai-assistant-quality-section"></div>');
		wrap.append(
			jQuery('<div class="ai-assistant-section-title"></div>').text(__("Recommendations"))
		);
		recommendations.forEach((recommendation) => {
			...
			if (recommendation.reason)
				card.append(jQuery("<div></div>").text(recommendation.reason));
			if (recommendation.action === "review_price_recommendation") {
				card.append(
					jQuery('<div class="text-muted small"></div>').text(
	...
	render_data_quality(quality) {
		...
		wrap.append(
			jQuery("<strong></strong>").text(
				__("Data quality: {0} — {1}%", [quality.grade || "unknown", score])
			)
		);
		if (quality.rows_analyzed != null) {
			wrap.append(
				jQuery('<span class="text-muted"></span>').text(
	...
	render_explainability(explainability) {
		...
		if (included.length) {
			...
			included.forEach((source) => {
				const rows =
					source.rows_analyzed == null
						? ""
						: " — " + source.rows_analyzed + " " + __("rows");
	...
	render_kpis(kpis) {
		...
		kpis.forEach((kpi) => {
			...
			let badge = "";
			if (kpi.severity) {
				badge = `<span class="ai-assistant-kpi-severity severity-${frappe.utils.escape_html(
					kpi.severity
				)}">${frappe.utils.escape_html(kpi.severity)}</span>`;
			}
			...
			if (kpi.comparison !== undefined && kpi.variance_pct !== undefined) {
				...
				comparison = `<div class="ai-assistant-kpi-comparison ${varianceClass}">${trendIcon} ${
					kpi.variance_pct >= 0 ? "+" : ""
				}${kpi.variance_pct}% vs ${compFormatted}</div>`;
			}

			const $card = $(`
                <div class="ai-assistant-kpi-card">
                    <div class="ai-assistant-kpi-label">${frappe.utils.escape_html(
						kpi.label
					)}</div>
                    ...
                    ${
						kpi.tooltip
							? `<div class="ai-assistant-kpi-tooltip">${frappe.utils.escape_html(
									kpi.tooltip
							  )}</div>`
							: ""
					}
	...
	build_chart_wrap(chart) {
		const $wrap = $(
			`<div class="ai-assistant-chart-wrap" data-chart-id="${frappe.utils.escape_html(
				chart.id
			)}"></div>`
		);
		if (chart.title) {
			$wrap.append(
				`<div class="ai-assistant-chart-title">${frappe.utils.escape_html(
					chart.title
				)}</div>`
			);
		}
	...
	init_chart(chart, $mount) {
		...
		try {
			...
			$mount.after(
				`<div class="text-muted small">${__("Chart could not be rendered")}</div>`
			);
		}
	}
	...
	init_matrix_chart(chart, mountElement) {
		const labels = (chart.data && chart.data.labels) || [];
		const values =
			((chart.data && chart.data.datasets && chart.data.datasets[0]) || {}).data || [];
		...
		labels.forEach((label, index) => {
			...
			cell.style.backgroundColor =
				"rgba(116, 62, 226, " + (0.12 + (0.78 * value) / maximum) + ")";
	...
	init_scatter_chart(chart, mountElement) {
		...
		labels.forEach((label, index) => {
			...
			title.textContent =
				label +
				": " +
				format_number(xValue, null, 2) +
				", " +
				format_number(yValue, null, 2) +
				"%";
	...
	render_table(table) {
		const $wrap = $(
			`<div class="ai-assistant-table-wrap" data-table-id="${frappe.utils.escape_html(
				table.id
			)}"></div>`
		);
		if (table.title) {
			...
			$titleRow.append(
				`<span class="ai-assistant-table-title">${frappe.utils.escape_html(
					table.title
				)}</span>`
			);
			const $toolbar = $(`
                <span class="ai-assistant-table-toolbar">
                    <button class="btn btn-xs btn-default ai-assistant-export-csv" title="${__(
						"Export CSV"
					)}">CSV</button>
                    <button class="btn btn-xs btn-default ai-assistant-export-xlsx" title="${__(
						"Export Excel"
					)}">XLSX</button>
                </span>
            `);
		...
		columns.forEach((col, idx) => {
			thead += `<th class="sortable" data-col-idx="${idx}" data-col-key="${frappe.utils.escape_html(
				col.key
			)}">${frappe.utils.escape_html(col.label)}</th>`;
		});
		thead += "</tr></thead>";
	...
	format_cell(val, col) {
		...
		if (type === "currency") {
			...
			return `<span class="ai-assistant-link" data-doctype="${frappe.utils.escape_html(
				col.doctype
			)}" data-name="${frappe.utils.escape_html(val)}">${frappe.utils.escape_html(
				val
			)}</span>`;
		}
			...
			const $item = $(`
                <div class="ai-assistant-insight" style="border-left-color:${borderColor};">
                    <div class="ai-assistant-insight-header">
                        <span class="ai-assistant-insight-type">${frappe.utils.escape_html(
							insight.type
						)}</span>
                        <span class="ai-assistant-insight-title">${frappe.utils.escape_html(
							insight.title
						)}</span>
                    </div>
                    <div class="ai-assistant-insight-desc">${this.markdown_lite(
						insight.description
					)}</div>
                    ${
						insight.actionable
							? `<div class="ai-assistant-insight-action text-success small">${__(
									"Actionable"
							  )}</div>`
							: ""
					}
		...
		warnings.forEach((w) => {
			const $item = $(`
                <div class="ai-assistant-warning">
                    <span class="ai-assistant-warning-code">${frappe.utils.escape_html(
						w.code
					)}</span>
                    <span class="ai-assistant-warning-msg">${frappe.utils.escape_html(
						w.message
					)}</span>
                    ${
						w.affected_metrics && w.affected_metrics.length
							? `<span class="ai-assistant-warning-metrics small text-muted">(${frappe.utils.escape_html(
									w.affected_metrics.join(", ")
							  )})</span>`
							: ""
					}
                </div>
	...
	render_sources(sources) {
		...
		return $(
			`<div class="ai-assistant-sources small text-muted">${__("Sources:")} ${names}</div>`
		);
	}
};
