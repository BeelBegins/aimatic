...
aimatic.RetailFinanceSetupPage = class RetailFinanceSetupPage {
	constructor(wrapper) {
		...
		this.can_initialize = (frappe.user_roles || []).some((role) =>
			["Accounts Manager", "System Manager"].includes(role)
		);
		this.build_filters();
	...
	bind_events() {
		...
		this.$root.on("click", "[data-rf-initialize-price-lists]", () =>
			this.initialize_branch_price_lists()
		);
	}
	...
	async refresh() {
		...
		try {
			this.data = await frappe.xcall("aimatic.retail_finance_setup.api.get_readiness", {
				company,
			});
			...
			console.error(error);
			const message =
				error && error.message ? error.message : __("Unable to load finance readiness.");
	...
	render() {
		...
		this.$root.html(`
            <section class="rf-hero">
                <div>
                    <div class="rf-eyebrow">${__("Registry version")} ${frappe.utils.escape_html(
            ...
            <section class="rf-checks">
                ...
                <div class="rf-check-grid">${data.checks
					.map((check) => this.check_card(check))
					.join("")}</div>
            </section>
            <section class="rf-capabilities">
                <div class="rf-section-heading">
                    <div>
                        ...
                        <p>${__(
							"Working, partial, and missing features remain visible here so future releases cannot silently omit them."
						)}</p>
                    </div>
	...
	check_card(check) {
		const details = (check.details || [])
			.map((detail) => `<li>${frappe.utils.escape_html(detail)}</li>`)
			.join("");
		const initializeAction =
			check.id === "stores" && this.can_initialize
				? `<button class="btn btn-xs btn-primary" data-rf-initialize-price-lists>${__(
						"Initialize branch price lists"
				  )}</button>`
				: "";
		return `
            <article class="rf-check-card">
                <div class="rf-card-title"><strong>${frappe.utils.escape_html(
					check.label
				)}</strong>${this.badge(check.status)}</div>
                ...
                <div class="rf-actions">
                    ...
                    ${
						check.route
							? `<button class="btn btn-xs btn-default" data-rf-route="${frappe.utils.escape_html(
									check.route
							  )}">${__("Open relevant records")}</button>`
							: ""
					}
                </div>
            </article>
	...
	initialize_branch_price_lists() {
		...
		frappe.confirm(
			__(
				"Create and link a selling-only Price List for every uninitialized branch in {0}?",
				[company]
			),
			async () => {
				...
				try {
					...
					frappe.show_alert(
						{
							message: __("Initialized {0}; already configured {1}.", [
								result.initialized_count,
								result.already_configured_count,
							]),
							indicator: "green",
						},
						7
					...
					frappe.msgprint({
						...
						message:
							error && error.message
								? error.message
								: __("Unable to initialize branch price lists."),
						indicator: "red",
					});
				} finally {
					frappe.dom.unfreeze();
	...
	render_capabilities() {
		...
		const rows = this.data.capabilities.filter(
			(row) =>
				(!status || row.readiness_status === status) && (!phase || row.phase === phase)
		);
		const html = rows.length
			? rows.map((row) => this.capability_card(row)).join("")
			: `<div class="rf-state">${__("No capabilities match these filters.")}</div>`;
		this.$root
			.find("[data-rf-capability-list]")
			.html(`<div class="rf-capability-list">${html}</div>`);
	}

	capability_card(row) {
		const details = (row.check_details || [])
			.map((detail) => `<li>${frappe.utils.escape_html(detail)}</li>`)
			.join("");
		return `
            <article class="rf-capability-card">
                <div class="rf-card-title">
                    <div>
                        ...
                        <div class="rf-meta">${frappe.utils.escape_html(
							row.category
						)} · ${frappe.utils.escape_html(row.phase)} · v${frappe.utils.escape_html(
			row.version
		)} · ${frappe.utils.escape_html(row.implementation_status)}</div>
                    </div>
                    ${this.badge(row.readiness_status)}
                </div>
                <p>${frappe.utils.escape_html(row.description)}</p>
                <div class="rf-actions">
                    <button class="btn btn-xs btn-default" data-rf-details="${frappe.utils.escape_html(
						row.id
					)}">${__("Guidance")}</button>
                    ${
						row.route
							? `<button class="btn btn-xs btn-default" data-rf-route="${frappe.utils.escape_html(
									row.route
							  )}">${__("Open records")}</button>`
							: ""
					}
                </div>
                <div class="rf-detail hidden" data-rf-detail-panel="${frappe.utils.escape_html(
					row.id
				)}">
                    <strong>${__("Required practice")}</strong>
                    <p>${frappe.utils.escape_html(row.guidance)}</p>
                    <p><strong>${__("Current result")}:</strong> ${frappe.utils.escape_html(
			row.readiness_message
		)}</p>
                    ${details ? `<ul>${details}</ul>` : ""}
                </div>
            </article>
