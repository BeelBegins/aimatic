...
aimatic.VendorPerformancePage = class VendorPerformancePage {
	...
	build_layout() {
		this.$container = $('<div class="vendor-performance-page"></div>').appendTo(
			this.page.body
		);
	...
	async refresh() {
		...
		try {
			const data = await frappe.xcall(
				"aimatic.vendor_performance.api.get_vendor_performance_summary",
				filters
			);
	...
	async load_drill_section(section) {
		...
		$body.html(
			`<div class="vp-loading"><div class="vp-state-icon">${frappe.utils.icon(
				"loader",
				"md"
			)}</div>${__("Loading...")}</div>`
		);
		try {
			...
			const message = frappe.utils.escape_html(
				error?.message || __("Unable to load this section.")
			);
	...
	render_error(error) {
		const message = frappe.utils.escape_html(
			error?.message || __("Unable to load vendor performance.")
		);
		this.$container.html(`
		...
		const noteHtml = `
            <div class="vp-note">
                ...
                <div class="vp-note-body">
                    <div><strong>${__("Stock values")}:</strong> ${frappe.utils.escape_html(
			data.stock_definition_note || ""
		)}</div>
		...
		this.$container.html(`
            ...
            <div class="vp-grid">
                ${this.card(
					"stock",
					"package",
					__("Supplier-SKU Exposure Stock (at Cost)"),
					this.money(summary.stock_value),
					`${this.number(summary.stock_qty)} ${__("qty")} · ${this.number(
						summary.stock_item_count
					)} ${__("items")}`
				)}
                ${this.card(
					"revenue",
					"trending-up",
					__("Sales Revenue (Window)"),
					this.money(summary.sales_amount),
					`${this.number(summary.sales_qty)} ${__("qty")} · ${this.number(
						summary.sales_doc_count
					)} ${__("docs")}`
				)}
                ${this.card(
					"cogs",
					"shopping-bag",
					__("Cost of Goods Sold (Window)"),
					this.money(summary.cogs_amount),
					`${this.number(summary.cogs_qty)} ${__("qty")}`
				)}
                ${this.card(
					"margin",
					"percent",
					__("Gross Margin (Window)"),
					this.money(summary.gross_margin_amount),
					`${this.percent(summary.gross_margin_pct)} ${__("of revenue")}`,
					summary.gross_margin_amount
				)}
                ${this.card(
					"purchase",
					"truck",
					__("Purchase Invoices (Window, at Cost)"),
					this.money(summary.purchase_amount),
					`${this.number(summary.purchase_qty)} ${__("qty")} · ${this.number(
						summary.purchase_doc_count
					)} ${__("docs")}`
				)}
                ${this.card(
					"receipt",
					"package-check",
					__("Purchase Receipts (Window, at Cost)"),
					this.money(summary.purchase_receipt_amount),
					`${this.number(summary.purchase_receipt_qty)} ${__("qty")} · ${this.number(
						summary.purchase_receipt_doc_count
					)} ${__("docs")}`
				)}
                ${this.card(
					"payable",
                ...
                ${this.card(
					...
					"credit-card",
					__("Last Payment"),
					lastPayment ? this.money(lastPayment.amount) : __("No payment"),
					lastPayment
						? `${frappe.datetime.str_to_user(
								lastPayment.posting_date
						  )} · ${frappe.utils.escape_html(lastPayment.payment_entry)}`
						: __("No submitted supplier payment found")
				)}
            </div>
            ${this.drill_section(
				"origin",
				"layers",
				__("Exact Remaining Stock by Vendor Origin (at Cost)"),
				`<span class="vp-badge">${__("Phase 2 FIFO attribution")}</span>`,
				data.origin_stock_definition_note,
				__("Compute exact vendor-origin stock")
			)}
            ${this.drill_section(
				"stock",
				"package",
				__("Current Stock of Supplier-Linked SKUs"),
				`<span class="vp-badge">${__("Top 15 by stock value")}</span>`,
				__(
					"Loads current stock (at cost) plus sales revenue, cost of goods sold, and margin in the selected window for each supplier-linked SKU."
				),
				__("Load stock items")
			)}
            ${this.drill_section(
				"purchases",
				"shopping-cart",
				__("Last 3 Purchase Invoices"),
				"",
				__("Loads the 3 most recent submitted purchase invoices for this supplier."),
				__("Load recent purchases")
			)}
            ${this.drill_section(
				"sales",
				"receipt",
				...
				__(
					"Loads the 3 most recent sales of supplier-linked SKUs within the selected window, with cost of goods sold and margin."
				),
				__("Load recent sales")
			)}
            ${this.drill_section(
				"ratios",
				"alert-triangle",
				...
				__("Check for abnormal sell-through")
			)}
        `);
	}
		...
		if (result.too_large) {
			return `
                <div class="vp-empty">
                    <div class="vp-state-icon">${frappe.utils.icon("alert-triangle", "md")}</div>
                    ${__(
						"This vendor has {0} stock ledger entries across its linked items, too many to replay on demand.",
						[this.number(result.sle_row_count)]
					)}
                    <div class="vp-item-meta">${frappe.utils.escape_html(
						summaryData.origin_stock_definition_note || ""
					)}</div>
                </div>
            `;
		}
		...
		return `
            <div class="vp-grid vp-drill-summary">
                ${this.card(
					"stock",
					"layers",
					__("Exact Vendor-Origin Stock (at Cost)"),
					this.money(summary.stock_value),
					`${this.number(summary.stock_qty)} ${__("qty")} · ${this.number(
						summary.item_count
					)} ${__("items")}`
	...
	renderOriginStockTable(rows, summary) {
		const unattributed = `${this.number(summary.unattributed_qty)} ${__("qty")} · ${this.money(
			summary.unattributed_value
		)}`;
		if (!rows.length) {
			return `
                <div class="vp-empty">
                    ...
                    <div class="vp-item-meta">${__(
						"Unattributed incoming stock currently in FIFO pool"
					)}: ${unattributed}</div>
                </div>
            `;
		}
		const body = rows
            ...
            <tr>
                ...
                <td>${
					row.last_origin_date ? frappe.datetime.str_to_user(row.last_origin_date) : ""
				}</td>
                <td>${
					row.last_origin_doc
						? this.docLink(
								row.last_origin_doctype || "Purchase Receipt",
								row.last_origin_doc
						  )
						: ""
				}</td>
        ...
        `
			...
			.join("");
		return `
            <div class="vp-item-meta" style="margin-bottom:8px;">${__(
				"Unattributed incoming stock currently in FIFO pool"
			)}: ${unattributed}</div>
	...
	render_ratios_section(result) {
		...
		const summaryHtml = `
            <div class="vp-note" style="margin-bottom:14px;">
                <div class="vp-note-icon">${frappe.utils.icon(
					flaggedCount ? "alert-triangle" : "info",
					"sm"
				)}</div>
                <div class="vp-note-body">
                    <div><strong>${__("Flagged items")}:</strong> ${flaggedCount} ${__("of")} ${
			items.length
		} (${criticalCount} ${__("critical")})</div>
                    <div class="vp-item-meta">${__(
						"Each item is judged against its own normal sell-through pace (from the trailing {0} days) given how long ago it was received - not a fixed ratio. Items without enough sales history for a personal pace are compared against this vendor's other items instead.",
						[this.number(result.velocity_baseline_days)]
					)}</div>
                </div>
            </div>
        `;
	...
	renderRatiosTable(rows) {
		if (!rows.length) {
			return `
                <div class="vp-empty">
                    <div class="vp-state-icon">${frappe.utils.icon("alert-triangle", "md")}</div>
                    ${__(
						"No purchase activity found for this supplier's linked items in the selected window."
					)}
                </div>
            `;
		}
		const statusBadge = (severity) => {
			if (severity === "critical") {
				return `<span class="vp-badge vp-badge-danger">${frappe.utils.icon(
					"alert-triangle",
					"xs"
				)} ${__("Critical")}</span>`;
			}
			if (severity === "abnormal") {
				return `<span class="vp-badge vp-badge-warning">${frappe.utils.icon(
					"alert-triangle",
					"xs"
				)} ${__("Abnormal")}</span>`;
			}
		...
		const body = rows
            ...
            <tr class="${
                ...
                <td>${
					row.purchase_date ? frappe.datetime.str_to_user(row.purchase_date) : ""
				} <span class="vp-item-meta">(${row.days_since_purchase}d ${__("ago")})</span></td>
                <td class="vp-num">${this.number(row.sold_qty)}</td>
                <td class="vp-num">${this.percent(row.pct_sold * 100)}</td>
                <td><span class="vp-badge">${
					row.method === "velocity" ? __("Own pace") : __("Peer compare")
				}</span></td>
	...
	renderStockTable(rows) {
		...
		const body = rows
            ...
            <tr>
                ...
                <td class="vp-num ${this.marginClass(row.gross_margin_in_window)}">${this.money(
					row.gross_margin_in_window
				)}</td>
                <td>${
					row.last_purchase_date
						? frappe.datetime.str_to_user(row.last_purchase_date)
						: ""
				}</td>
                <td>${
					row.last_purchase_doc
						? this.docLink(
								row.last_purchase_doctype || "Purchase Invoice",
								row.last_purchase_doc
						  )
						: ""
				}</td>
                <td class="vp-num">${
					row.last_purchase_rate ? this.money(row.last_purchase_rate) : ""
				}</td>
            </tr>
        `
			)
	...
	renderSalesTable(rows) {
		if (!rows.length) {
			return `
                <div class="vp-empty">
                    ...
                    ${__(
						"No submitted sales found for this supplier-linked SKU set in the selected window."
					)}
                </div>
            `;
		}
		const body = rows
            ...
            <tr>
                ...
                <td class="vp-num ${this.marginClass(row.gross_margin)}">${this.money(
					row.gross_margin
				)}</td>
