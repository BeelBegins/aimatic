...
def check_company(company):
	...
	message = (
		"Company accounting foundation is configured."
		if not missing
...
def check_stores(company):
	...
	if not branches:
		return _result(
			"stores", "Store mappings", "blocked", "No Branch is configured for this company.", "List/Branch"
		)
	...
	for row in branches:
		...
		if not cost_center:
			...
			"Cost Center", {"name": cost_center, "company": company, "is_group": 0, "disabled": 0}
		):
		...
		if not warehouse:
			...
			_has_field("Warehouse", "custom_branch")
			and frappe.db.get_value("Warehouse", warehouse, "custom_branch") != branch.name
		):
		...
		if not price_list:
			...
			if not price_list_details:
				...
				not price_list_details.selling or price_list_details.buying or not price_list_details.enabled
			):
	...
	message = (
		f"{len(branches)} store mapping(s) are ready."
		if not issues
		else f"{len(issues)} store setup issue(s) require attention."
	)
...
def check_pos(company):
	if not _has_field("POS Profile", "branch"):
		return _result(
			"pos",
			"POS accounting context",
			"blocked",
			"The Branch accounting dimension is not installed on POS Profile.",
			"List/Accounting Dimension",
		)
	profiles = frappe.get_all(
		"POS Profile",
		filters={"company": company, "disabled": 0},
		fields=["name", "warehouse", "cost_center", "branch"],
	)
	if not profiles:
		return _result(
			"pos",
			"POS accounting context",
			"warning",
			"No enabled POS Profile is configured.",
			"List/POS Profile",
		)
	issues = []
	for profile in profiles:
		...
		if not profile.warehouse:
			...
			"Warehouse", {"name": profile.warehouse, "company": company, "is_group": 0, "disabled": 0}
		):
			issues.append(f"{profile.name}: warehouse is not an enabled leaf")
		elif (
			profile.branch
			and _has_field("Warehouse", "custom_branch")
			and frappe.db.get_value("Warehouse", profile.warehouse, "custom_branch") != profile.branch
		):
		...
		if not profile.cost_center:
			...
			"Cost Center", {"name": profile.cost_center, "company": company, "is_group": 0, "disabled": 0}
		):
			issues.append(f"{profile.name}: cost center is not an enabled leaf")
	status = "blocked" if issues else "pass"
	message = (
		f"{len(profiles)} enabled POS Profile(s) have accounting context."
		if not issues
		else f"{len(issues)} POS mapping issue(s) require attention."
	)
...
def check_cash_bank(company):
	...
	mappings = frappe.db.count(
		"Mode of Payment Account", {"company": company, "default_account": ["is", "set"]}
	)
	...
	message = (
		"Cash, bank, and payment mappings are available."
		if not missing
		else "Missing or incomplete: " + ", ".join(missing) + "."
	)
...
def check_receivables_payables(company):
	...
	status = "blocked" if missing else "pass"
	message = (
		"Receivable and payable control accounts are available."
		if not missing
		else "Missing: " + ", ".join(missing) + "."
	)
...
def check_inventory(company):
	...
	status = "blocked" if missing else "pass"
	message = (
		f"{stock_items} stock item(s) and {warehouses} branch warehouse(s) are available."
		if not missing
		else "Missing: " + ", ".join(missing) + "."
	)
	return _result(
		"inventory", "Inventory valuation foundation", status, message, "query-report/Stock Balance"
	)


def check_tax(company):
	settings = frappe.db.count("FBR Integration Settings", {"company": company, "enabled": 1})
	tax_accounts = frappe.db.count(
		"Account", {"company": company, "is_group": 0, "disabled": 0, "account_type": "Tax"}
	)
	if settings and tax_accounts:
		return _result(
			...
			"Tax configuration",
			"pass",
			f"{settings} enabled FBR setting(s) and {tax_accounts} tax account(s) are configured.",
			"List/FBR Integration Settings",
		)
	return _result(
		"tax",
		"Tax configuration",
		"warning",
		"Tax applicability or configuration needs review; this does not change existing tax records.",
		"List/FBR Integration Settings",
	)
...
def check_reporting(company):
	...
	if not _has_field("GL Entry", "branch"):
		return _result(
			"reporting",
			"Forward branch reporting",
			"blocked",
			"The Branch accounting dimension is not installed on GL Entry.",
			"List/Accounting Dimension",
		)
	...
	if not fiscal_year:
		return _result(
			...
			"Forward branch reporting",
			"warning",
			"No fiscal year covers today, so forward reporting cannot be checked.",
			"List/Fiscal Year",
		)
	...
	if missing_branch:
		return _result(
			...
			"Forward branch reporting",
			"warning",
			f"{missing_branch} current-fiscal-year income/expense GL row(s) have no Branch. Opening entries and unavailable history are not backfilled.",
	...
	return _result(
		...
		"Forward branch reporting",
		"pass",
		"Current-fiscal-year income and expense GL rows are Branch-tagged. Opening entries remain the accepted cutover baseline.",
		"query-report/General Ledger",
	)
