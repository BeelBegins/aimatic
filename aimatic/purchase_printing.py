...
def _build_latest_history_map(item_codes: list[str], current_doctype: str, current_name: str | None) -> dict[str, dict]:
    rows = []
    rows.extend(
...
def get_purchase_print_item_context(doc) -> dict[str, dict]:
    ...
    for row in doc.items or []:
        ...
        context[row.name] = {
            ...
            "old_sale_price": flt(history.get("custom_shelf_price") or history.get("custom_mrp") or history.get("rate")),
        }
