...
def _settings(require_enabled=True):
	...
	price_list = doc.price_list or branch.get("default_selling_price_list") or frappe.db.get_single_value("Selling Settings", "selling_price_list")
	if not price_list:
		frappe.throw(_("Shopping has no selling Price List"))
	return doc, frappe._dict(branch=branch.name, company=branch.company, warehouse=warehouse, price_list=price_list, currency=frappe.db.get_value("Price List", price_list, "currency") or frappe.db.get_value("Company", branch.company, "default_currency"))
...
def _customer_for_user():
	...
	customer = frappe.db.get_value("Portal User", {"user": user, "parenttype": "Customer", "parentfield": "portal_users"}, "parent")
	if not customer:
		frappe.throw(_("This account is not linked to a Customer. Ask store support to link it."), frappe.PermissionError)
...
def _product_rows(search=None, category=None, brand=None, item_code=None, offset=0, limit=30):
	...
	rows = frappe.get_all("Shopping Product", filters=filters, fields=["item", "category", "featured", "public_name", "public_description", "image", "display_order"], order_by="display_order asc, modified desc", limit_start=cint(offset), limit_page_length=min(max(cint(limit) or 30, 1), 60))
	items = {row.name: row for row in frappe.get_all("Item", filters={"name": ["in", [x.item for x in rows] or [""]], "disabled": 0, "is_sales_item": 1}, fields=["name", "item_name", "description", "brand", "stock_uom", "image"])}
	result = []
	for product in rows:
		...
		if search and search.lower() not in f"{product.item} {name} {product.category} {item.brand or ''}".lower():
			continue
...
def _price_and_stock(item_code, context, price_list=None):
	...
	bin_row = frappe.db.get_value("Bin", {"item_code": item_code, "warehouse": context.warehouse}, ["actual_qty", "reserved_qty"], as_dict=True) or {}
...
def _public_product(product, item, context, price_list=None):
	...
	return {"item_code": item.name, "item_name": product.public_name or item.item_name, "description": product.public_description or item.description or "", "image_url": product.image or item.image or "", "category": product.category, "brand": item.brand or "", "uom": item.stock_uom, "rate": rate, "currency": context.currency, "available": available > 0, "available_qty": available, "featured": bool(product.featured)}
...
def _make_order(customer, items):
	...
	doc = frappe.get_doc({"doctype": "Sales Order", "customer": customer, "company": context.company, "branch": context.branch, "set_warehouse": context.warehouse, "selling_price_list": price_list, "delivery_date": delivery_date, "order_type": "Sales", "remarks": f"Ai Matic Shopping – {_DELIVERY_METHOD} / {_PAYMENT_METHOD}\n{settings.pickup_instructions or ''}"})
	...
	for item in parsed:
		...
		doc.append("items", {"item_code": item["item_code"], "qty": item["qty"], "uom": item["uom"], "warehouse": context.warehouse, "delivery_date": delivery_date})
...
def get_storefront(branch=None):
	...
	return {"store_name": settings.store_name, "branch": context.branch, "currency": context.currency, "categories": [{"name": value, "label": value} for value in categories], "delivery_methods": [{"name": _DELIVERY_METHOD, "label": _DELIVERY_METHOD, "instructions": settings.pickup_instructions or ""}], "payment_methods": [{"name": _PAYMENT_METHOD, "label": _PAYMENT_METHOD}]}
...
def search_products(branch=None, category=None, brand=None, search=None, offset=0, limit=30):
	...
	return {"products": [_public_product(product, item, context) for product, item in _product_rows(search, category, brand, offset=offset, limit=limit)]}
...
def quote_cart(branch=None, delivery_method=None, address_name=None, items=None):
	...
	payload = {"nonce": secrets.token_urlsafe(12), "user": frappe.session.user, "customer": customer, "items": parsed, "grand_total": flt(doc.grand_total, 2), "expires_at": str(expires)}
	return {"quoteToken": _quote_token(payload), "currency": doc.currency, "subtotal": flt(doc.net_total, 2), "discount": flt(doc.discount_amount, 2), "taxes": flt(doc.total_taxes_and_charges, 2), "deliveryCharge": 0, "grandTotal": flt(doc.grand_total, 2), "expiresAt": str(expires), "lines": [{"lineId": item["line_id"], "itemCode": row.item_code, "quantity": flt(row.qty), "rate": flt(row.rate, 2), "amount": flt(row.amount, 2), "available": True} for item, row in zip(parsed, doc.items)]}
...
def place_order(request_id, quote_token, address_name=None, delivery_method=None, payment_method=None):
	...
	existing = frappe.db.get_value("Shopping Order Request", request_id, ["user", "sales_order"], as_dict=True)
	if existing:
	...
	frappe.get_doc({"doctype": "Shopping Order Request", "request_id": request_id, "user": user, "customer": customer, "delivery_method": delivery_method, "payment_method": payment_method, "status": "Processing", "created_at": now_datetime()}).insert(ignore_permissions=True)
	...
	return {"sales_order": doc.name, "status": doc.status, "grand_total": flt(doc.grand_total, 2), "currency": doc.currency, "duplicate": False}


@frappe.whitelist()
def get_orders(offset=0, limit=20):
	_user, customer = _customer_for_user()
	return {"orders": frappe.get_all("Sales Order", filters={"customer": customer}, fields=["name", "transaction_date", "delivery_date", "status", "currency", "grand_total", "modified"], order_by="modified desc", limit_start=cint(offset), limit_page_length=min(max(cint(limit) or 20, 1), 50))}


@frappe.whitelist()
def get_order_status(order):
	_user, customer = _customer_for_user()
	row = frappe.db.get_value("Sales Order", {"name": order, "customer": customer}, ["name", "transaction_date", "delivery_date", "status", "currency", "grand_total", "modified"], as_dict=True)
	if not row:
