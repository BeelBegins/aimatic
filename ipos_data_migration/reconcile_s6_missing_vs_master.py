"""Read-only: can the 4,897 S6-missing rows be created from Ho-MasterItemFilea16c3b.xlsx?
...
audit_s6_barcodes_vs_szl.py's reasoning, unlike the S7 script's single hard
target. Does not create Items / groups / tax categories / brands. Writes
Excel reports under /tmp and uploads to Home/Migrations/S6.
...
any S6 SubCatName not covered by this table still blocks as unknown_subcat
...
MISSING_XLSX = "/tmp/s6_barcode_audit_20260912_235236/s6_missing_items_20260912_235236.xlsx"
MISSING_CSV = "/tmp/s6_barcode_audit_20260912_235236/s6_missing_items_20260912_235236.csv"
RESULT_FOLDER = "Home/Migrations/S6"
UPLOAD = True
...
def _latest_missing_xlsx() -> str:
	matches = sorted(Path("/tmp").glob("s6_barcode_audit_*/s6_missing_items_*.xlsx"))
	if matches:
		return str(matches[-1])
	if Path(MISSING_XLSX).is_file():
		return MISSING_XLSX
	frappe.throw("Missing barcode-audit output. Run audit_s6_barcodes_vs_szl.py first.")
...
def main():
	...
	for m in missing:
		...
		row_out = {
			"S6_Barcode1": m["Barcode1"],
			"S6_Barcode2": m["Barcode2"],
			"S6_Description": m["Description"],
			"S6_Onhand": m["Onhand"],
			"S6_CurCost": m["CurCost"],
			"S6_SalePrice": m["SalePrice"],
	...
	out_dir = Path(f"/tmp/s6_master_reconcile_{stamp}")
	...
	ready_path = out_dir / f"s6_create_ready_{stamp}.xlsx"
		...
		blocked_norm.append(
			{
				...
				"S6_Barcode1": r.get("S6_Barcode1") or r.get("Barcode1", ""),
				"S6_Barcode2": r.get("S6_Barcode2") or r.get("Barcode2", ""),
				"S6_Description": r.get("S6_Description") or r.get("Description", ""),
				"S6_Onhand": r.get("S6_Onhand") or r.get("Onhand", ""),
	...
	blocked_path = out_dir / f"s6_create_blocked_{stamp}.xlsx"
	write_xlsx(blocked_path, blocked_headers, [[r.get(h, "") for h in blocked_headers] for r in blocked_norm])

	gates_path = out_dir / f"s6_approval_gates_{stamp}.xlsx"
	wb = Workbook()
	ws = wb.active
	...
	summary_path = out_dir / f"s6_master_reconcile_summary_{stamp}.json"
