...
def _create_leaf_account(
	company, account_name, parent_account, root_type, account_number=None, account_type=None
):
...
def set_company_defaults(company, head_office_cost_center):
	...
	for fieldname, account_name in ref.COMPANY_DEFAULTS.items():
		...
		if not account:
			print(
				f"WARNING: could not resolve account '{account_name}' for Company field '{fieldname}' -- skipped"
			)
			continue
