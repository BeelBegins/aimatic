"""Attach S5 orphan Barcode2 onto the already-matched Item (live szl).
...
barcode rows/UOMs. New row copies UOM from that item's existing barcode rows.
    ...
    ns={}
    path="/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/attach_s5_orphan_barcode2.py"
...
TARGET_SITE = "szl"
ORPHAN_XLSX = "/tmp/s5_barcode_audit_20260912_235236/s5_orphan_barcode2_20260912_235236.xlsx"
...
def _latest_orphan_xlsx() -> str:
	matches = sorted(Path("/tmp").glob("s5_barcode_audit_*/s5_orphan_barcode2_*.xlsx"))
	if matches:
		return str(matches[-1])
	if Path(ORPHAN_XLSX).is_file():
		return ORPHAN_XLSX
	frappe.throw("Missing orphan audit output. Run audit_s5_barcodes_vs_szl.py first.")
