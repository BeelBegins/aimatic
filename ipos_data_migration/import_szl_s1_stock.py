...
def create_item_group_tree(all_parents, all_subcats, subcat_parent):
	...
	frappe.db.commit()
	print(
		f"Item Group tree (new-item categories only): {len(all_parents)} categories, {len(all_subcats)} subcategories, {created} newly created"
	)
...
def ensure_stock_prereqs():
	...
	if settings.valuation_method != "Moving Average":
		print(
			f"Setting Stock Settings.valuation_method to Moving Average (was {settings.valuation_method!r})"
		)
		settings.valuation_method = "Moving Average"
		changed = True
	if not settings.allow_negative_stock:
		print(
			"Enabling Stock Settings.allow_negative_stock (required for the negative-Onhand Material Issue rows)"
		)
	...
	if stale:
		print(
			f"Clearing default_warehouse on {len(stale)} Item Default rows still pointing at 'Stores - SSM'"
		)
		for name in stale:
...
def create_stock_entries(stock_rows, disabled_items):
	...
	for start in range(0, len(stock_rows), STOCK_ENTRY_CHUNK_SIZE):
		chunk = [
			r
			for r in stock_rows[start : start + STOCK_ENTRY_CHUNK_SIZE]
			if r["item_code"] not in disabled_items
		]
	...
	print(
		f"Stock rows with a genuinely zero rate (allow_zero_valuation_rate=1): {zero_rate_rows} of {len(stock_rows)}"
	)
	if skipped_disabled:
		print(
			f"SKIPPED {len(skipped_disabled)} positive stock rows for disabled Items (needs a human decision):"
		)
...
def create_negative_stock_entries(stock_rows, disabled_items):
	...
	if skipped_disabled:
		print(
			f"SKIPPED {len(skipped_disabled)} negative stock rows for disabled Items (needs a human decision):"
		)
		for r in skipped_disabled:
...
def run():
	...
	print(
		f"*** DRY_RUN = {DRY_RUN} *** Loaded {len(rows)} rows from {FILE_PATH}, posting date {POSTING_DATE}"
	)
	...
	print(
		f"Matched existing Item: {len(rows) - len(unmatched_rows)}; unmatched (new Item needed): {len(unmatched_rows)}"
	)
	...
	for index, row in enumerate(rows, start=1):
		try:
			...
			item_name, created_new, skip_reason = create_item(
				row, item_group, fbr_categories, existing_barcodes
			)
			if skip_reason:
				stats["skipped_items"] += 1
				failures.append(
					{"row": row["_rownum"], "description": row.get("Description"), "reason": skip_reason}
				)
				continue
			...
			rate = exclusive_rate(as_float(row.get("CurCost")), get_item_tax_rate(item_name, tax_rate_cache))
			if qty > 0:
			...
			stats["failed_rows"] += 1
			failures.append(
				{"row": row["_rownum"], "description": row.get("Description"), "reason": str(exc)}
			)
