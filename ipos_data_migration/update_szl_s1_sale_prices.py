...
def parse_rows(path):
    with zipfile.ZipFile(path) as archive:
        ...
        for row_node in worksheet.findall("a:sheetData/a:row", NS):
            ...
            if str(record.get("ItemCode", "")).strip() or str(
...
def run():
    if frappe.local.site != TARGET_SITE:
        frappe.throw(
            f"Locked to site {TARGET_SITE!r}; current site is {frappe.local.site!r}."
        )
    ...
    for barcode, parent in frappe.db.sql(
        "SELECT barcode, parent FROM `tabItem Barcode` "
    ...
    for name, item_code, rate in frappe.db.sql(
        "SELECT name, item_code, price_list_rate FROM `tabItem Price` "
        "WHERE price_list=%s",
        PRICE_LIST,
    ):
    ...
    for row in parse_rows(FILE_PATH):
        ...
        barcodes = [
            str(row.get(field, "")).strip() for field in ("ItemCode", "RefCode")
        ]
        matched = set()
    ...
    for action, name, item_code, rate in actions:
        if action == "update":
            frappe.db.set_value(
                "Item Price", name, "price_list_rate", rate, update_modified=True
            )
        else:
