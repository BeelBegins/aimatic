"""Read-only: can the 4,897 S4-missing rows be created from Ho-MasterItemFiled5b674.xlsx?
...
audit_s4_barcodes_vs_szl.py's reasoning, unlike the S7 script's single hard
target. Does not create Items / groups / tax categories / brands. Writes
Excel reports under /tmp and uploads to Home/Migrations/S4.
...
any S4 SubCatName not covered by this table still blocks as unknown_subcat
...
MASTER_FILE_ID = "9872249e71"
MISSING_XLSX = "/tmp/s4_barcode_audit_20260912_235236/s4_missing_items_20260912_235236.xlsx"
MISSING_CSV = "/tmp/s4_barcode_audit_20260912_235236/s4_missing_items_20260912_235236.csv"
RESULT_FOLDER = "Home/Migrations/S4"
UPLOAD = True
...
def main():
	...
	for m in missing:
		...
		row_out = {
			"S4_Barcode1": m["Barcode1"],
			"S4_Barcode2": m["Barcode2"],
			"S4_Description": m["Description"],
			"S4_Onhand": m["Onhand"],
			"S4_CurCost": m["CurCost"],
			"S4_SalePrice": m["SalePrice"],
	...
	out_dir = Path(f"/tmp/s4_master_reconcile_{stamp}")
	...
	ready_path = out_dir / f"s4_create_ready_{stamp}.xlsx"
		...
		blocked_norm.append(
			{
				...
				"S4_Barcode1": r.get("S4_Barcode1") or r.get("Barcode1", ""),
				"S4_Barcode2": r.get("S4_Barcode2") or r.get("Barcode2", ""),
				"S4_Description": r.get("S4_Description") or r.get("Description", ""),
				"S4_Onhand": r.get("S4_Onhand") or r.get("Onhand", ""),
	...
	blocked_path = out_dir / f"s4_create_blocked_{stamp}.xlsx"
	write_xlsx(blocked_path, blocked_headers, [[r.get(h, "") for h in blocked_headers] for r in blocked_norm])

	gates_path = out_dir / f"s4_approval_gates_{stamp}.xlsx"
	wb = Workbook()
	ws = wb.active
	...
	summary = {
		"stamp": stamp,
		"site": site,
		"master_file": "Ho-MasterItemFiled5b674.xlsx",
		"master_file_id": MASTER_FILE_ID,
		"master_rows": len(master_rows),
		"s4_missing_rows": len(missing),
	...
	summary_path = out_dir / f"s4_master_reconcile_summary_{stamp}.json"
