"""S5 Sector C vendor opening balances.
...
name, not NTN, and are posted.
    ...
    ns = {}
    path = "/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/import_s5_vendor_balances.py"
...
TARGET_SITE = "szl"
FILE_PATH = "/home/nabeel/frappe-bench/sites/szl/private/files/1005Vendorbalances.xlsx"
COMPANY = "Siezal Supermarket"
BRANCH = "S5 - Sector C"
POSTING_DATE = "2026-09-20"
REFERENCE_PREFIX = "LEGACY-OB-S5-"
...
def _assert_site():
	...
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S5 vendor import: expected {TARGET_SITE}, got {site}")
...
def _ensure_supplier(row, stats):
	...
	if row["reason"] == "no_clean_ntn":
		...
		if not supplier_name:
			supplier_name = f"S5 LEGACY CODE {row['code']}"
...
def _post_opening(row, supplier, payable_account, temp_opening_account, cost_center, stats, failures):
	...
	if existing:
		...
		return [existing]
	remark = (
		f"Legacy S5 vendor {row['name']} (Code {row['code']}, Ledger {row['ledger_code']}, "
	...
	try:
		...
		if blank:
			frappe.throw(f"{je.name} missing S5 dimensions")
		frappe.db.commit()
