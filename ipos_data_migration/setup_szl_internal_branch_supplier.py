...
def main():
    ...
    missing_branches = [b["branch_name"] for b in ref.BRANCHES if not frappe.db.exists("Branch", b["branch_name"])]
    if missing_branches:
        frappe.throw(
            f"Branches not yet set up: {missing_branches} -- run setup_szl_branches.main() first."
        )
