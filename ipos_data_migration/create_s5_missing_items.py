"""Create the S5-missing Items on siezal (mock). Master file only.
...
User decisions 2026-09-13 (see s5_wallayat_complex.md sign-off log):
...
- FRUITES & VEGETABLES -> Fruits & Vegetables: approved new leaf Item Group
  ...
  ensure_s5_fruits_vegetables_group.py, already run on siezal).
...
Run:
    ns={}
    path="/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/create_s5_missing_items.py"
...
STOCK_XLS = "/home/nabeel/frappe-bench/sites/szl/private/files/1005stockposition.xls"
DRY_RUN = True
...
def _assert_site():
	...
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S5 item create: expected site '{TARGET_SITE}', got '{site}'")
...
def _parse_s5_rows() -> list[dict]:
	if not Path(STOCK_XLS).is_file():
		frappe.throw(f"S5 stock file missing: {STOCK_XLS}")
