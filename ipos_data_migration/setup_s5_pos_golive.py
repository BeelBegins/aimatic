"""S5 Sector C POS go-live masters on live szl.
...
``account_for_change_amount`` (S7 cash must not remain on S5). Idempotent.

    ns = {}
    path = "/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/setup_s5_pos_golive.py"
...
SELLING_PRICE_LIST = "S5 - Sector C Selling Price List"
FOODPANDA_PRICE_LIST = "S5 - Sector C Foodpanda Price List"
CASH_ACCOUNT_NUMBER = "1115"
CASH_ACCOUNT_NAME = "Cash in Hand - S5SC"
PARENT_CASH = "1100 - Cash In Hand - SSM"
MODE_OF_PAYMENT = "Cash - S5SC"
WALK_IN = "S5 Walk in Customer"
FOODPANDA_CUSTOMER = "S5 Food Panda"
COUNTER_PROFILE = "S5 Counter 1"
FOODPANDA_PROFILE = "S5 Food Panda"
...
def _assert_site():
	...
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S5 POS go-live: expected {TARGET_SITE}, got {site}")
	for doctype, name in (
		("Branch", BRANCH),
		("Warehouse", WAREHOUSE),
	...
	print(
		{
			...
			"fbr_pos_id_unchanged": frappe.db.get_value(
				...
				"Siezal Supermarket-S5 - Sector C",
				"pos_id",
			),
		}
	)
