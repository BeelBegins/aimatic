"""Read-only audit: S6 (Khalid Block) stock-position barcodes vs catalog.

Source: sites/szl/private/files/1006stockposition.xls (File e195467e67),
uploaded 2026-09-20. "1006" is S6's own FBR-style branch code, same
...
Run:
    ...
    exec(open("apps/aimatic/ipos_data_migration/audit_s6_barcodes_vs_szl.py").read())
    main()
"""
...
def main():
	...
	out_dir = Path(f"/tmp/s6_barcode_audit_{stamp}")
	out_dir.mkdir(parents=True, exist_ok=True)

	miss_headers = ["Barcode1", "Barcode2", "Description", "Onhand", "CurCost", "SalePrice", "Total"]
	miss_path = out_dir / f"s6_missing_items_{stamp}.xlsx"
	...
	abs_path = out_dir / f"s6_absent_barcodes_{stamp}.xlsx"
	_write_xlsx(
		abs_path,
	...
	orphan_path = out_dir / f"s6_orphan_barcode2_{stamp}.xlsx"
	_write_xlsx(
		orphan_path,
		orphan_headers,
		[
	...
	summary = {
		"stamp": stamp,
		"site": site,
		"source_file": "1006stockposition.xls",
	...
	summary_path = out_dir / f"s6_barcode_summary_{stamp}.json"
