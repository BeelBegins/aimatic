"""S4 Wallayat Complex selling prices and opening stock (mock first).

Source: 1004stockposition.xls
- Sale Price -> S4 - Wallayat Complex Selling Price List
- Quantity + tax-exclusive Cost Price -> S4 - Wallayat Complex - SSM
...
Hard-targets ``siezal`` for the mock pass. Change TARGET_SITE only during the
separately approved live cutover, after a fresh ``szl`` backup and a dry run
whose totals match the signed-off mock.

Run from ``bench --site siezal console`` using one explicit namespace (IPython
otherwise gives nested exec() a split globals/locals namespace):

    ns = {}
    path = "/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/import_s4_prices_and_stock.py"
    exec(compile(open(path).read(), path, "exec"), ns)
    ns["main"]()            # dry-run
    ns["main"](apply=True)  # mock apply, only after explicit approval
"""
...
WAREHOUSE = "S4 - Wallayat Complex - SSM"
COST_CENTER = "S4 - Wallayat Complex - SSM"
PRICE_LIST = "S4 - Wallayat Complex Selling Price List"
UOM = "Pcs"
...
POSITIVE_TAG = "S4-ONHAND-IMPORT-2026-09-13"
NEGATIVE_TAG = "S4-ONHAND-NEGATIVE-IMPORT-2026-09-13"
...
APPROVED_EXCLUSIONS = {
	"6224700269510": "BAKING SHEETS 24PCS",
	"13": "TEST2",
	"10010": "TEST5",
	"6224700269813": "CLING FILM 100SQ",
	"6261422503119": "RANI MANGO BOTTLE 200ML",
	"6224700269817": "CLING FILM 200ST",
	"1001": "TEST",
	"8961100096855": "UJAB BIN ROLL NO-40",
	"12": "TEST1",
	"6224700269801": "WAXPAPER 32 SQ",
	"6261422503744": "RANI PEACH BOTTLE 200ML",
	"6224700269503": "BAKING SHEETS 36PCS",
	"14": "TEST3",
}

# User-reviewed S4 duplicate-row price choices. Quantities are combined;
# the chosen price is explicit rather than inferred with a blanket rule.
APPROVED_SALE_PRICES = {
	"STO-ITEM-2026-11934": 1319,
	"STO-ITEM-2026-00408": 145,
	"STO-ITEM-2026-07622": 279,
	"STO-ITEM-2026-09772": 175,
	"STO-ITEM-2026-07635": 550,
	"STO-ITEM-2026-10488": 188,
	"STO-ITEM-2026-16756": 285,
	"STO-ITEM-2026-13825": 2785,
	"STO-ITEM-2026-07636": 585,
	"STO-ITEM-2026-05547": 85,
	"STO-ITEM-2026-10247": 230,
	"STO-ITEM-2026-09541": 219,
	"STO-ITEM-2026-06127": 819,
	"STO-ITEM-2026-10487": 185,
	"STO-ITEM-2026-07637": 625,
	"STO-ITEM-2026-07981": 150,
	"STO-ITEM-2026-12451": 1085,
	"STO-ITEM-2026-11587": 60,
	"STO-ITEM-2026-10318": 1235,
	"STO-ITEM-2026-11933": 499,
	"STO-ITEM-2026-21145": 279,
	"STO-ITEM-2026-16573": 1920,
	"STO-ITEM-2026-00407": 285,
	"STO-ITEM-2026-17790": 2759,
	"STO-ITEM-2026-00406": 285,
}

# User-approved zero-quantity pairs: combine the duplicate source rows, post
# no stock, and discard only their cost residual. Never generalize this rule.
...
def _assert_site_and_source():
	...
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S4 price/stock import: expected site {TARGET_SITE}, got {site}")
	path = Path(SOURCE_XLS)
	if not path.is_file():
		frappe.throw(f"S4 source missing: {SOURCE_XLS}")
	digest = hashlib.sha256(path.read_bytes()).hexdigest()
	if digest != SOURCE_SHA256:
		frappe.throw(f"S4 source hash changed: expected {SOURCE_SHA256}, got {digest}")
...
def parse_s4_rows() -> list[dict]:
	workbook = xlrd.open_workbook(SOURCE_XLS)
	sheet = workbook.sheet_by_index(0)
	header = [_source_text(value) for value in sheet.row_values(0)]
	expected_header = ["Barcode1", "Description", "Quantity", "Cost Price", "Sale Price", "Barcode2", "TotalCost"]
	if header[:7] != expected_header:
		frappe.throw(f"Unexpected S4 source header: {header[:7]}")
	...
	if len(rows) != EXPECTED_SOURCE_ROWS:
		frappe.throw(f"Unexpected S4 source row count: expected {EXPECTED_SOURCE_ROWS}, got {len(rows)}")
	return rows
...
def _assert_accounting_context():
	...
	if branch_cost_center != COST_CENTER:
		frappe.throw(f"Unexpected S4 cost center: expected {COST_CENTER}, got {branch_cost_center}")
...
def resolve_item(row, barcode_exact, item_exact, variant):
	...
	S4's six reviewed Barcode1/Barcode2 collisions are genuinely different
...
def build_plan(rows):
	...
	for row in rows:
		...
		if not item_code:
			if resolution == "unmatched" and _is_approved_exclusion(row):
...
def _assert_opening_state(plan):
	...
	existing = frappe.db.sql(
		"SELECT name, remarks, docstatus FROM `tabStock Entry` WHERE remarks LIKE 'S4-ONHAND-%'",
		as_dict=True,
	)
	unexpected = [row for row in existing if row.remarks not in expected_tags]
	if unexpected:
		frappe.throw(f"Unexpected S4 opening tags already exist: {unexpected[:10]}")
	...
	if duplicate_tags:
		frappe.throw(f"Duplicate submitted S4 opening tags: {duplicate_tags}")
	...
	if nonzero_bins and not any(row.docstatus == 1 for row in existing):
		frappe.throw(f"{WAREHOUSE} already has {nonzero_bins} nonzero bins without S4 opening tags")
...
def _submit_chunks(rows, *, positive: bool):
	...
	for start in range(0, len(rows), STOCK_ENTRY_CHUNK_SIZE):
		...
		if blank_detail or blank_gl:
			frappe.throw(
				f"{entry.name} missing S4 dimensions: detail={blank_detail}, gl={blank_gl}"
			)
		frappe.db.commit()
		created += 1
