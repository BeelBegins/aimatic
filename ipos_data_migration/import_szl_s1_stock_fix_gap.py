...
def run():
	...
	if DRY_RUN:
		print(
			f"Would need to create {len(resolved)} still-missing Items (not counting {len(failures)} failures)."
		)
	...
	print(
		f"Items with no Bin row for {WAREHOUSE} yet (need a stock entry): {len(missing)} of {len(item_codes)}"
	)
