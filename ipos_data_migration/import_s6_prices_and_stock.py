"""S6 Khalid Block selling prices and opening stock (mock first).

Source: 1006stockposition.xls
- Sale Price -> S6 - Khalid Block Selling Price List
- Quantity + tax-exclusive Cost Price -> S6 - Khalid Block - SSM
...
otherwise gives nested exec() a split globals/locals namespace):
    ...
    ns = {}
    path = "/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/import_s6_prices_and_stock.py"
...
SOURCE_SHA256 = "46e04873fc4d7299e7452bc02dae1a57c70c55ed1a34be768a518b71bf9af8f5"
EXPECTED_SOURCE_ROWS = 10679

COMPANY = "Siezal Supermarket"
BRANCH = "S6 - Khalid Block"
WAREHOUSE = "S6 - Khalid Block - SSM"
COST_CENTER = "S6 - Khalid Block - SSM"
PRICE_LIST = "S6 - Khalid Block Selling Price List"
UOM = "Pcs"
...
POSITIVE_TAG = "S6-ONHAND-IMPORT-2026-09-20"
NEGATIVE_TAG = "S6-ONHAND-NEGATIVE-IMPORT-2026-09-20"
...
APPROVED_EXCLUSIONS = {
	...
	"800033300685": "BARLAY WHOLE 400G",
	...
	# Short local code 22 pads onto Accu-Check / Humulin barcodes. Not those Items.
	"22": "LASTIC LARGE BLACK",
}

# Duplicate source rows: combine qty; take the higher S6 selling price
# (S4 sign-off 2026-09-13, reused here).
...
APPROVED_ZERO_NET_STOCK_ITEMS = set()


def _assert_site_and_source():
	site = getattr(frappe.local, "site", None) or ""
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S6 price/stock import: expected site {TARGET_SITE}, got {site}")
	path = Path(SOURCE_XLS)
	if not path.is_file():
		frappe.throw(f"S6 source missing: {SOURCE_XLS}")
	digest = hashlib.sha256(path.read_bytes()).hexdigest()
	if digest != SOURCE_SHA256:
		frappe.throw(f"S6 source hash changed: expected {SOURCE_SHA256}, got {digest}")
...
def parse_s6_rows() -> list[dict]:
	...
	if len(rows) != EXPECTED_SOURCE_ROWS:
		frappe.throw(f"Unexpected S6 source row count: expected {EXPECTED_SOURCE_ROWS}, got {len(rows)}")
	return rows
...
def _assert_accounting_context():
	...
	if branch_cost_center != COST_CENTER:
		frappe.throw(f"Unexpected S6 cost center: expected {COST_CENTER}, got {branch_cost_center}")
...
def resolve_item(row, barcode_exact, item_exact, variant):
	...
	S6's six reviewed Barcode1/Barcode2 collisions are genuinely different
...
def _assert_opening_state(plan):
	...
	existing = frappe.db.sql(
		"SELECT name, remarks, docstatus FROM `tabStock Entry` WHERE remarks LIKE 'S6-ONHAND-%'",
		as_dict=True,
	)
	unexpected = [row for row in existing if row.remarks not in expected_tags]
	if unexpected:
		frappe.throw(f"Unexpected S6 opening tags already exist: {unexpected[:10]}")
	...
	if duplicate_tags:
		frappe.throw(f"Duplicate submitted S6 opening tags: {duplicate_tags}")
	...
	if nonzero_bins and not any(row.docstatus == 1 for row in existing):
		frappe.throw(f"{WAREHOUSE} already has {nonzero_bins} nonzero bins without S6 opening tags")
...
def _submit_chunks(rows, *, positive: bool):
	...
	for start in range(0, len(rows), STOCK_ENTRY_CHUNK_SIZE):
		...
		if blank_detail or blank_gl:
			frappe.throw(
				f"{entry.name} missing S6 dimensions: detail={blank_detail}, gl={blank_gl}"
			)
		frappe.db.commit()
		created += 1
