"""Create the 548 S7-missing Items on siezal (mock). Master file only.

Hard-target: siezal. DRY_RUN True by default. No warehouse / item_defaults.
No prices, no stock. ERPNext item_code from autoname hook (do not pre-call
make_autoname). Unmatched brands left empty. No new Item Groups / FBR / Brands.

Run:
    ns={}
    path="/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/create_s7_missing_items.py"
...
STOCK_UOM = "Pcs"
# 2026-09-06: fresh master + stock files (replace the stale 2026-09-04 pair).
# Mock-verified clean on siezal (546 created, 0 blocked) before this live run.
MASTER_PATH = "/home/nabeel/frappe-bench/sites/szl/private/files/Ho-MasterItemFilebaf350.xlsx"
S7_XLS = "/home/nabeel/frappe-bench/sites/szl/private/files/1007.xls"
...
def _assert_site():
	...
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S7 item create: expected site '{TARGET_SITE}', got '{site}'")
...
def _parse_s7_rows() -> list[dict]:
	path = Path(S7_CSV)
	if not path.is_file():
		frappe.throw(f"S7 CSV missing at {S7_CSV}; convert the .xls first")
	rows = []
...
def _plan_rows():
	...
	for m in s7:
		...
		if _exists(b1, variant_to_item) or _exists(b2, variant_to_item):
			continue
		if (m["Description"] or "").strip().upper() in JUNK_DESCRIPTIONS:
			skipped.append({**m, "reason": "junk_description"})
			continue
		...
		if len(hits) > 1:
			...
			continue
		rec = hits[0]
		group = _resolve_group(str(rec.get("SubCatName") or ""), group_by_lower)
		fbr = _resolve_fbr(str(rec.get("Fbr_Tax_Category") or ""), fbr_set, fbr_lower)
		if not group:
			blocked.append({**m, "reason": f"unknown_subcat:{rec.get('SubCatName')}"})
			continue
		if not fbr:
...
def main(apply: bool = False):
	...
	if not apply:
		print("DRY_RUN — no inserts. Pass apply=True to create on siezal.")
	...
	series = frappe.db.sql(
		"select name, current from tabSeries where name like %s", ("STO-ITEM%",)
	)
	print(
		{
			"created": len(created),
