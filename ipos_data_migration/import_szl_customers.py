...
def run():
    ...
    print(f"Loaded {len(rows)} legacy rows -> {len(groups)} customer groups ({len(merged_groups)} merged by mobile)")
    print(f"Total legacy LoyaltyPoints across all rows: {total_points}")
    for mobile, members in list(merged_groups.items())[:20]:
        print("MERGE", mobile, [(m["customer_code"], m["customer_name"], m["loyalty_points"]) for m in members])
