...
FILE_PATH = "/home/nabeel/frappe-bench/sites/siezal/public/files/vendordataghouritown.xlsx"
...
def get_or_create_wht_payable_account(company):
	...
	if not parent:
		frappe.throw(
			f"No 'Duties and Taxes' group account found under {company} to attach the WHT payable account to."
		)
			...
			conflicts.append(
				{
					...
					"members": [
						(m["supplier_code"], m["supplier_name"], m["fbrtype"], m["wht_rate"]) for m in members
					],
				}
			)
...
def run():
	...
	code_to_supplier, supplier_stats, conflicts, supplier_failures = create_suppliers(
		groups, company, wht_account
	)
