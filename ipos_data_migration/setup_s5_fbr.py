"""Copy FBR Integration Settings for S5 - Sector C on live szl.

Copies the S4 row, sets branch and branch_code=1005, and leaves the
new row **disabled** with pos_id empty until a unique production POS ID is
set. Does not change S1/S4/S7 FBR rows.

    ns={}
    path="/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/setup_s5_fbr.py"
...
NAME = "Siezal Supermarket-S5 - Sector C"
BRANCH = "S5 - Sector C"
BRANCH_CODE = "1005"
PROTECTED = {
...
def _assert_site():
    ...
    if site != TARGET_SITE:
        frappe.throw(f"Refusing S5 FBR setup: expected {TARGET_SITE}, got {site}")
...
def main(apply: bool = False):
    ...
    doc.branch_code = BRANCH_CODE
    doc.enabled = 0
    doc.pos_id = 100500  # placeholder, not a live POS ID; leave disabled
