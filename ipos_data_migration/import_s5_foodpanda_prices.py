"""S5 Foodpanda prices from the portal export productsS5.xlsx.
...
Writes only ``S5 - Sector C Foodpanda Price List``. Hard-targets
``szl``. PKR only. SHA256-locked source. Dry-run by default.

    ns = {}
    path = "/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/import_s5_foodpanda_prices.py"
...
TARGET_SITE = "szl"
BRANCH = "S5 - Sector C"
PRICE_LIST = "S5 - Sector C Foodpanda Price List"
REQUIRED_CURRENCY = "PKR"
SOURCE_XLSX = "/home/nabeel/frappe-bench/sites/szl/private/files/productsS5.xlsx"
SOURCE_FILE_URL = "/private/files/productsS5.xlsx"
SOURCE_SHA256 = "4eac93fe445b713de71a93d4e84b7a54c825fae25769cddde3a72c437a44ce91"
...
PROTECTED_PRICE_LISTS = {
	...
	"S6 - Khalid Block Foodpanda Price List",
...
def _assert_site():
	...
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S5 Foodpanda import: expected {TARGET_SITE}, got {site}")
	path = Path(SOURCE_XLSX)
	if not path.is_file():
		frappe.throw(f"Missing Foodpanda source {SOURCE_XLSX}")
	digest = hashlib.sha256(path.read_bytes()).hexdigest()
	if digest != SOURCE_SHA256:
		frappe.throw(f"S5 Foodpanda source hash changed: expected {SOURCE_SHA256}, got {digest}")
...
def parse_rows():
	...
	if len(rows) != EXPECTED_SOURCE_ROWS:
		frappe.throw(f"S5 Foodpanda row count changed: expected {EXPECTED_SOURCE_ROWS}, got {len(rows)}")
	return rows
	...
	print(
		{
			...
			"current_s5_foodpanda_prices": frappe.db.count("Item Price", {"price_list": PRICE_LIST}),
