"""Create the S6-missing Items on siezal (mock). Master file only.

Adapted from create_s4_missing_items.py. Hard-target: live szl. DRY_RUN via the apply= kwarg. No warehouse /
...
User decisions 2026-09-13 (see s6_wallayat_complex.md sign-off log):
...
- FRUITES & VEGETABLES -> Fruits & Vegetables: approved new leaf Item Group
  ...
  ensure_s6_fruits_vegetables_group.py, already run on siezal).
...
Run:
    ns={}
    path="/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/create_s6_missing_items.py"
    exec(compile(open(path).read(), path, "exec"), ns)
    ns["main"]()           # dry-run
    ns["main"](apply=True) # insert on szl
"""
...
STOCK_XLS = "/home/nabeel/frappe-bench/sites/szl/private/files/1006stockposition.xls"
DRY_RUN = True
...
def _assert_site():
	...
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S6 item create: expected site '{TARGET_SITE}', got '{site}'")
...
def _parse_s6_rows() -> list[dict]:
	if not Path(STOCK_XLS).is_file():
		frappe.throw(f"S6 stock file missing: {STOCK_XLS}")
	wb = xlrd.open_workbook(STOCK_XLS)
