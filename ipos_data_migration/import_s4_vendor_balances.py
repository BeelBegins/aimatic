"""S4 Wallayat Complex vendor opening balances.
...
name, not NTN, and are posted.
    ...
    ns = {}
    path = "/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/import_s4_vendor_balances.py"
...
TARGET_SITE = "szl"
FILE_PATH = "/home/nabeel/frappe-bench/sites/szl/private/files/1004vendorbalances.xlsx"
COMPANY = "Siezal Supermarket"
BRANCH = "S4 - Wallayat Complex"
POSTING_DATE = "2026-09-13"
REFERENCE_PREFIX = "LEGACY-OB-S4-"
...
def _assert_site():
	...
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S4 vendor import: expected {TARGET_SITE}, got {site}")
...
def _ensure_supplier(row, stats):
	...
	if row["reason"] == "no_clean_ntn":
		...
		if not supplier_name:
			supplier_name = f"S4 LEGACY CODE {row['code']}"
...
def _repair_suffix_duplicates(stats: Counter):
	_cancel_and_delete_duplicate(
		"RAUF TRADING (DELI SOGA) (5479929)",
		"RAUF TRADING (DELI SOGA)",
		"5479929",
		stats,
	)
	_cancel_and_delete_duplicate(
		"PRIME DISTRIBUTION NETWORK (ISLAMABAD TEA) (I249921)",
		"PRIME DISTRIBUTION NETWORK (ISLAMABAD TEA)",
		None,
		stats,
	)
...
def _post_opening(row, supplier, payable_account, temp_opening_account, cost_center, stats, failures):
	...
	if existing:
		...
		return [existing]
	remark = (
		f"Legacy S4 vendor {row['name']} (Code {row['code']}, Ledger {row['ledger_code']}, "
	...
	try:
		...
		if blank:
			frappe.throw(f"{je.name} missing S4 dimensions")
		frappe.db.commit()
