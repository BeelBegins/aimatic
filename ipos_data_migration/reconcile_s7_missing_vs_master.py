"""Read-only: can the 548 S7-missing rows be created from Ho-MasterItemFile.xlsx?

Hard-targets szl. Does not create Items / groups / tax categories / brands.
Writes Excel reports under /tmp and uploads to Home/Migrations/S7.
...
TARGET_SITE = "szl"
# 2026-09-06: fresh master file (File 5f223eab4b, replaces stale fdd3fa1cd5)
# and the missing-items list from the re-run audit against the fresh 1007.xls
# stock file (see audit_s7_barcodes_vs_szl.py).
MASTER_PATH = "/home/nabeel/frappe-bench/sites/szl/private/files/Ho-MasterItemFilebaf350.xlsx"
MASTER_FILE_ID = "5f223eab4b"
MISSING_XLSX = "/tmp/s7_barcode_audit_20260906_023530/s7_missing_items_20260906_023530.xlsx"
# Fallback: rebuild missing from prior CSV if xlsx path moved
MISSING_CSV = "/tmp/s7_barcode_audit_20260906_023530/s7_missing_items_20260906_023530.csv"
...
EXISTING_GROUP_ALIASES = {
	...
	# Approved 2026-09-04: vendor dump maps to the cosmetics leaf under
	# Face-Hair-Body - Oral Care (not the parent group; items need a leaf).
	"HAJI MOEEN (COSMETICS)": "Colour Cosmetics",
	# Approved 2026-09-04: create Item Group Tobacco (leaf), then alias here.
	"TOBACCO": "Tobacco",
}
...
def resolve_item_group(subcat: str, group_by_lower: dict[str, str]):
	"""Existing groups only. Aliases must already exist on site."""
...
def resolve_brand(brand: str, brand_exact: dict[str, str], brand_norm: dict[str, list[str]]):
	"""Use an existing Brand only on exact / case-insensitive name match.

	No new Brand masters. No alphanumeric/fuzzy merge. Unmatched or
	ambiguous → leave Item.brand empty (not a create-block).
	"""
	name = (brand or "").strip()
	if not name or name.upper() == "NULL":
...
def main():
	...
	for m in missing:
		...
		if not hits:
			...
			continue
		if len(hits) > 1:
			# Prefer exact ItemCode match on Barcode1
		...
		if fbr_st not in ("ok", "ok_casefold"):
			reasons.append(fbr_st if fbr_st.startswith("blank") or fbr_st.startswith("unknown") else fbr_st)
			if fbr_st == "unknown_fbr":
				reasons[-1] = f"unknown_fbr:{fbr_raw}"
		# Unmatched brands are left empty — never a create-block.

		row_out = {
			"S7_Barcode1": m["Barcode1"],
			"S7_Barcode2": m["Barcode2"],
			"S7_Description": m["Description"],
			"S7_Onhand": m["Onhand"],
			"S7_CurCost": m["CurCost"],
			"S7_SalePrice": m["SalePrice"],
	...
	# Unique unresolved subcats / fbr / new brands among blocked-in-master
	...
	brand_left_blank_rows = sum(
		1 for r in ready if r.get("brand_status") == "left_blank_no_existing_match"
	)

	stamp = now_datetime().strftime("%Y%m%d_%H%M%S")
	out_dir = Path(f"/tmp/s7_master_reconcile_{stamp}")
	...
	ready_path = out_dir / f"s7_create_ready_{stamp}.xlsx"
	...
	# normalize blocked rows to have keys
	blocked_norm = []
	for r in blocked:
		blocked_norm.append(
			{
				"block_reason": r.get("block_reason", ""),
				"S7_Barcode1": r.get("S7_Barcode1") or r.get("Barcode1", ""),
				"S7_Barcode2": r.get("S7_Barcode2") or r.get("Barcode2", ""),
				"S7_Description": r.get("S7_Description") or r.get("Description", ""),
				"S7_Onhand": r.get("S7_Onhand") or r.get("Onhand", ""),
	...
	blocked_path = out_dir / f"s7_create_blocked_{stamp}.xlsx"
	write_xlsx(
		blocked_path,
		blocked_headers,
		[[r.get(h, "") for h in blocked_headers] for r in blocked_norm],
	)

	gates_path = out_dir / f"s7_approval_gates_{stamp}.xlsx"
	wb = Workbook()
	ws = wb.active
	...
	summary = {
		"stamp": stamp,
		"master_file": "Ho-MasterItemFilebaf350.xlsx",
		"master_file_id": MASTER_FILE_ID,
		"master_rows": len(master_rows),
		"s7_missing_rows": len(missing),
		...
		"verdict": (
			...
			else "PARTIAL_READY_NEEDS_APPROVAL_OR_MASTER_FIX"
		),
		"files": {
	...
	summary_path = out_dir / f"s7_master_reconcile_summary_{stamp}.json"
