"""S4 Wallayat Complex POS go-live masters on live szl.
...
``account_for_change_amount`` (S7 cash must not remain on S4). Idempotent.

    ns = {}
    path = "/home/nabeel/frappe-bench/apps/aimatic/ipos_data_migration/setup_s4_pos_golive.py"
...
SELLING_PRICE_LIST = "S4 - Wallayat Complex Selling Price List"
FOODPANDA_PRICE_LIST = "S4 - Wallayat Complex Foodpanda Price List"
CASH_ACCOUNT_NUMBER = "1113"
CASH_ACCOUNT_NAME = "Cash in Hand - S4WC"
PARENT_CASH = "1100 - Cash In Hand - SSM"
MODE_OF_PAYMENT = "Cash - S4WC"
WALK_IN = "S4 Walk in Customer"
FOODPANDA_CUSTOMER = "S4 Food Panda"
COUNTER_PROFILE = "S4 Counter 1"
FOODPANDA_PROFILE = "S4 Food Panda"
TEMPLATE_CASH_ACCOUNT = "1112 - Cash in Hand - S7EH - SSM"
TEMPLATE_COUNTER = "S7 Counter 1"
TEMPLATE_FOODPANDA = "S7 Food Panda"


def _assert_site():
	site = getattr(frappe.local, "site", None) or ""
	if site != TARGET_SITE:
		frappe.throw(f"Refusing S4 POS go-live: expected {TARGET_SITE}, got {site}")
	for doctype, name in (
		("Branch", BRANCH),
		("Warehouse", WAREHOUSE),
...
def main():
	...
	_ensure_customer(WALK_IN, SELLING_PRICE_LIST, "S7 Walk in Customer")
	_ensure_customer(FOODPANDA_CUSTOMER, FOODPANDA_PRICE_LIST, "S7 Food Panda")
	_copy_profile(
		TEMPLATE_COUNTER,
		COUNTER_PROFILE,
		payments=[
