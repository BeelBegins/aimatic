"""Read-only audit: S4 (Wallayat Complex) stock-position barcodes vs catalog.

Source: sites/szl/private/files/1004stockposition.xls (File 3b594c134a),
uploaded 2026-09-12. "1004" is S4's own FBR-style branch code, same
...
Run:
    ...
    exec(open("apps/aimatic/ipos_data_migration/audit_s4_barcodes_vs_szl.py").read())
    main()
"""
...
def main():
	...
	out_dir = Path(f"/tmp/s4_barcode_audit_{stamp}")
	out_dir.mkdir(parents=True, exist_ok=True)

	miss_headers = ["Barcode1", "Barcode2", "Description", "Onhand", "CurCost", "SalePrice", "Total"]
	miss_path = out_dir / f"s4_missing_items_{stamp}.xlsx"
	...
	abs_path = out_dir / f"s4_absent_barcodes_{stamp}.xlsx"
	_write_xlsx(
		abs_path,
	...
	orphan_path = out_dir / f"s4_orphan_barcode2_{stamp}.xlsx"
	_write_xlsx(
		orphan_path,
		orphan_headers,
		[
	...
	summary = {
		"stamp": stamp,
		"site": site,
		"source_file": "1004stockposition.xls",
	...
	summary_path = out_dir / f"s4_barcode_summary_{stamp}.json"
